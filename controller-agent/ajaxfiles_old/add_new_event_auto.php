<?php
   ...
   $sport_id = $conn->real_escape_string($_REQUEST['sport_id']);
   $event_id = $conn->real_escape_string($_REQUEST['event_id']);
   $market_id = $conn->real_escape_string($_REQUEST['market_id']);
   $event_name = $conn->real_escape_string($_REQUEST['event_name']);
   $match_date = $conn->real_escape_string($_REQUEST['match_date']);
   $match_date = date('Y-m-d H:i:s',(($match_date)?$match_date:time()));
   $match_max = $conn->real_escape_string($_REQUEST['match_max']);
   $bookmaker_max = $conn->real_escape_string($_REQUEST['bookmaker_max']);
   $net_exposure_limit = $conn->real_escape_string($_REQUEST['net_exposure_limit']);
