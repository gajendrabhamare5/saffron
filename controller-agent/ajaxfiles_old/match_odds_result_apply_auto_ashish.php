...
$get_market_data = $conn->query("select * from bet_details where $condition event_id=$event_id and market_id=$market_id LIMIT 1");
$fetch_get_market_data = mysqli_fetch_assoc($get_market_data);
$runnerName = $fetch_get_market_data['market_name'];

$datetime = date("Y-m-d H:i:s");
$query2 = "INSERT INTO `result_match_odds` (`eventId`,`selectionId`,`runnerName`,`status`,`datetime`,`added_by`,`ip_address`,`user_agent`,`is_bookmaker`) VALUES ($event_id,$market_id,'$runnerName',$status,'$datetime','$user_id','$ip_address','$user_agent','0')";
...
if ($market_type == "Match Odds" or $market_type == "Toss Odds" or $market_type == "Tie Odds" or $market_type == "Other Odds") {
    ...
    if ($market_type == "Match Odds") {
        ...
        $get_market_type = $conn->query("select * from bet_details where $condition event_id='$event_id' and market_id='$market_id' and market_type NOT IN ('MATCH_ODDS','TOSS_ODDS','TIE_ODDS','FANCY_ODDS')");
    ...
    $get_market_data = $conn->query("select group_concat(user_id) all_users from bet_details where $condition event_id=$event_id and market_id=$market_id and market_type='$market_type_search'");
    $fetch_get_market_data = mysqli_fetch_assoc($get_market_data);
    ...
    if ($result_status == 0) {
        ...
        $get_win_bet_details_1 = $conn->query("select * from bet_details where $condition event_id=$event_id and market_id=$market_id and market_type='$market_type_search' and bet_type='$bet_type' and bet_status=1");
        ...
        $get_loss_bet_details_1 = $conn->query("select * from bet_details where $condition event_id=$event_id and market_id<>$market_id and market_type='$market_type_search' and bet_type='$bet_type' and bet_status=1");
        ...
        $get_win_bet_details_2 = $conn->query("select * from bet_details where $condition event_id=$event_id and market_id<>$market_id and market_type='$market_type_search' and bet_type='$bet_type_2' and bet_status=1");
        ...
        $get_loss_bet_details_2 = $conn->query("select * from bet_details where $condition event_id=$event_id and market_id=$market_id and market_type='$market_type_search' and bet_type='$bet_type_2' and bet_status=1");
        ...
        $get_loss_bet_details_1 = $conn->query("select * from bet_details where $condition event_id=$event_id and market_id=$market_id and market_type='$market_type_search' and bet_type='$bet_type_2' and bet_status=1");
        while ($fetch_get_loss_bet_details_1 = mysqli_fetch_assoc($get_loss_bet_details_1)) {
            //loss entry
    ...
    $get_all_user_commision = $conn->query("select bet_id,event_id,market_id,event_name,market_name,market_type,SUM(bet_result) as overal_result,user_id,Email_ID from bet_details as b left outer join user_login_master as u on u.UserID=b.user_id where $condition1 b.event_id='$event_id' and b.market_type='MATCH_ODDS' and b.bet_status=0 GROUP BY user_id");
		...
		if (true) {
           if ($comm_amount > 0) {
                ...
                */
                ...
                $level_pers = get_level_per_result_new($conn, $commision_user_id,$user_list);
