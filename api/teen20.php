...
if ($result == 1) {
    $result_status = 'A';
    $market_id[] = 1;
} else if ($result == "CAN" or $result == 0) {
    $result_status = '';
	...
	$conn->query("insert into twenty_teenpatti_result_new (event_id,game_type,result_status,cards,result_time,desc_remakrs,data) VALUES('$mid','$game_type','$result_status','$cards3','$result_time','$rdesc','$data_original')");
	$get_all_bet1 = $conn->query("select * from bet_teen_details where event_id='$mid' and bet_status=1 and event_type='$event_type'");
    while ($fetch_get_all_bet1 = mysqli_fetch_assoc($get_all_bet1)) {
        $bet_id = $fetch_get_all_bet1['bet_id'];
        $update_bet = $conn->query("update bet_teen_details set bet_status=2,bet_result='0',bet_win_amount='0' where bet_id='$bet_id' and bet_status=1");
...
$event_type = "2020TEENPATTI";

$market_id = array();
$market_id[] = $result_status;
$result_list=explode("#",$remark);
if (strpos($result_list[1], "Player A") !== false) { 
    $market_id[] = 5;
}
if (strpos($result_list[1], "Player B") !== false) { 
    $market_id[] = 6;
}
if (strpos($result_list[2], "Player A") !== false) { 
    $market_id[] = 11;
}
if (strpos($result_list[2], "Player B") !== false) { 
    $market_id[] = 12;
}
if (strpos($result_list[3], "A : Pair") !== false || strpos($result_list[3], "A : Straight") !== false || strpos($result_list[3], "A : Flush") !== false || strpos($result_list[3], "A : Trio") !== false || strpos($result_list[3], "A : Straight Flush") !== false) { 
    $market_id[] = 3;
}
if (strpos($result_list[3], "B : Pair") !== false || strpos($result_list[3], "B : Straight") !== false || strpos($result_list[3], "B : Flush") !== false || strpos($result_list[3], "B : Trio") !== false || strpos($result_list[3], "B : Straight Flush") !== false) { 
    $market_id[] = 4;
}
if (strpos($result_list[4], "A : Black") !== false) { 
    $market_id[] = 7;
}
if (strpos($result_list[4], "B : Black") !== false) { 
    $market_id[] = 9;
}
if (strpos($result_list[4], "A : Red") !== false) { 
    $market_id[] =8;
}
if (strpos($result_list[4], "B : Red") !== false) { 
    $market_id[] =10;
}
...
if(empty($result)){
     $get_all_bet1 = $conn->query("select * from bet_teen_details where event_id='$mid' and bet_status=1 and event_type='$event_type' and market_id in (1,2)");
    while ($fetch_get_all_bet1 = mysqli_fetch_assoc($get_all_bet1)) {
        $bet_id = $fetch_get_all_bet1['bet_id'];
        $update_bet = $conn->query("update bet_teen_details set bet_status=2,bet_result='0',bet_win_amount='0' where bet_id='$bet_id' and bet_status=1");
        ...
        $conn->query("delete from accounts where bet_id='$bet_id' and game_type=1");
        $conn->query("delete from accounts_temp where bet_id='$bet_id' and game_type=1");
        $conn->query("delete from exposure_details where event_id='$mid' and market_type ='$event_type' and market_id in (1,2)");
    }
}
...
while ($fetch_get_all_bet = mysqli_fetch_assoc($get_all_bet)) {
    ...
    if (strpos($result_list[2], "Tie") !== false && ($bet_market_id == "11" || $bet_market_id == "12")) { 
        $winner_flag=false;
    }
    ...
    if ($winner_flag) { 
        ...
        if($bet_market_id == 3){
             if (strpos($result_list[3], "A : Straight Flush") !== false) { 
                $actual_win_amount2 = $actual_win_amount2 * 40;
            }
            else if (strpos($result_list[3], "A : Flush") !== false) { 
                $actual_win_amount2 = $actual_win_amount2 * 4;
            }
            else if (strpos($result_list[3], "A : Trio") !== false) { 
                $actual_win_amount2 = $actual_win_amount2 * 30;
            }
            else if (strpos($result_list[3], "A : Straight") !== false) { 
                $actual_win_amount2 = $actual_win_amount2 * 6;
            }
        }
        if($bet_market_id == 4){
            if (strpos($result_list[3], "B : Straight Flush") !== false) { 
                $actual_win_amount2 = $actual_win_amount2 * 40;
            }
            else if (strpos($result_list[3], "B : Flush") !== false) { 
                $actual_win_amount2 = $actual_win_amount2 * 4;
            }
            else if (strpos($result_list[3], "B : Trio") !== false) { 
                $actual_win_amount2 = $actual_win_amount2 * 30;
            }
            else if (strpos($result_list[3], "B : Straight") !== false) { 
                $actual_win_amount2 = $actual_win_amount2 * 6;
            }
        }
