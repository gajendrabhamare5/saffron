...
if ($cards3 != "") {
    $check_already = $conn->query("select * from twenty_teenpatti_result where event_id='$mid' and game_type='$game_type'");
    $fetch_check_already = mysqli_fetch_assoc($check_already);
    if ($fetch_check_already != null) {
        $tt_result_id = $fetch_check_already['tt_result_id'];

        $conn->query("update twenty_teenpatti_result set event_id='$mid',game_type='$game_type',result_status='$result_status',cards='$cards3',result_time='$result_time' where tt_result_id='$tt_result_id'");
    }
}
