...
$is_sa_card_win = is_same_card($joker_card,$sa_card,$cards);
if($is_sa_card_win == true){
	$market_id[] = 1;
}

$is_sb_card_win = is_same_card($joker_card,$sb_card,$cards);
if($is_sb_card_win == true){
	$market_id[] = 4;
}



$is_even = is_even($joker_card, $cards);
if ($is_even) {
    $market_id[] = 25;
} else {
    $market_id[] = 24;
...
$is_spade = is_spade($joker_card, $cards);
if($is_spade){
	$market_id[] = 20;
}

$is_club = is_club($joker_card, $cards);
if($is_club){
	$market_id[] = 21;
}

$is_heart = is_heart($joker_card, $cards);
if($is_heart){
	$market_id[] = 22;
}
$is_diamond = is_diamond($joker_card, $cards);
if($is_diamond){
	$market_id[] = 23;
}
...
$dragon_cards_type_id = cards_type_id($joker_card, $cards);
...
function is_diamond($card11, $cards) {
    $card1 = $cards->$card11;
    $type = $card1['type'];
    if ($type == "diamond") {
        return true;
...
function is_heart($card11, $cards) {
    $card1 = $cards->$card11;
    $type = $card1['type'];
    if ($type == "heart") {
        return true;
...
function is_spade($card11, $cards) {
    $card1 = $cards->$card11;
    $type = $card1['type'];
    if ($type == "spade") {
        return true;
...
function cards_type_id($card11, $cards, $type_cards=null) {
    $card1 = $cards->$card11;
    $rank = $card1['rank'];
...
function is_same_card($joker_card,$first_card,$cards){
	$card1 = $cards->$joker_card;
	$card2 = $cards->$first_card;
	
	$first_card_win = $card1['rank'];
	$second_card_win = $card2['rank'];
	
	if($first_card_win == $second_card_win){
		return true;
	}
	else{
		return false;
