...
$sum_cards = sum_cards($cards2, $cards);
...
while ($fetch_get_all_bet = mysqli_fetch_assoc($get_all_bet)) {
    ...
    if (in_array($bet_market_id, $market_id)) {
		...
		if($conn->affected_rows > 0){
			...
			if(INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $lossing_amount != 0)){
			...
			if(INSERT_ACCOUNTS_TEMP && (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $lossing_amount != 0))){
				$conn->query("insert into accounts_temp (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`event_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`game_type`,`transaction_id`) VALUES('$bet_user_id','0','$account_description','$bet_id','$mid','-$lossing_amount','Debit','7','$transaction_time','1',1,'$transaction_id')");
			}
...
function sum_cards($cards2, $cards) {
	...
	foreach($cards2 as $c){
		...
		if(($rank_1 >= 10 && $spade10 == false) || $spade9 == true){
			$high += $rank_1;
		}
		else{
			$low += $rank_1;
