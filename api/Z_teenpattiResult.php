...
if ($game_type == "teenmuflis") {
    $event_type = "MUFLIS";
    $mid = str_replace("16.", "", $mid);
} else if ($game_type == "teen20") {
    $event_type = "2020TEENPATTI";
    $mid = str_replace("11.", "", $mid);
}
...
$a_cards = array($cards2[0], $cards2[1], $cards2[2]);
$b_cards = array($cards2[3], $cards2[4], $cards2[5]);
$is_a_trio = is_trio($a_cards[0], $a_cards[1], $a_cards[2], $cards);
if ($is_a_trio) {
    $market_id[] = 2;
    $return_odds = 35;
} else {
    $is_a_straight_flush = is_straight_flush($a_cards[0], $a_cards[1], $a_cards[2], $cards);
    if ($is_a_straight_flush) {
        $market_id[] = 2;
        $return_odds = 45;
    } else {
        $is_a_flush = is_flush($a_cards[0], $a_cards[1], $a_cards[2], $cards);
        if ($is_a_flush) {
            //ron
            $market_id[] = 2;
            $return_odds = 6;
        } else {
            $is_a_color = is_color($a_cards[0], $a_cards[1], $a_cards[2], $cards);
            if ($is_a_color) {

                $market_id[] = 2;
                $return_odds = 4;
            } else {
                $is_a_pair = is_pair($a_cards[0], $a_cards[1], $a_cards[2], $cards);
                if ($is_a_pair) {
                    $market_id[] = 2;
                    $return_odds = 1;
                } else {
...
$is_b_trio = is_trio($b_cards[0], $b_cards[1], $b_cards[2], $cards);
if ($is_b_trio) {
    $market_id[] = 4;
    $return_odds2 = 35;
} else {
    $is_b_straight_flush = is_straight_flush($b_cards[0], $b_cards[1], $b_cards[2], $cards);
    if ($is_b_straight_flush) {
        $market_id[] = 4;
        $return_odds2 = 45;
    } else {
        $is_b_flush = is_flush($b_cards[0], $b_cards[1], $b_cards[2], $cards);
        if ($is_b_flush) {
            $market_id[] = 4;
            $return_odds2 = 6;
        } else {
            $is_b_color = is_color($b_cards[0], $b_cards[1], $b_cards[2], $cards);
            if ($is_b_color) {
                $market_id[] = 4;
                $return_odds2 = 4;
            } else {
                $is_b_pair = is_pair($b_cards[0], $b_cards[1], $b_cards[2], $cards);
                if ($is_b_pair) {
                    $market_id[] = 4;
                    $return_odds2 = 1;
                } else {
...
while ($fetch_get_all_bet = mysqli_fetch_assoc($get_all_bet)) {
    ...
    if (in_array($bet_market_id, $market_id)) {
        if ($event_type == "2020TEENPATTI") {
            if ($bet_market_id == 2) {
                $actual_win_amount2 = $return_odds * $bet_amount;
            } else if ($bet_market_id == 4) {
