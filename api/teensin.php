...
if ($result_list[2] == "Player A") {
    $market_id[] = 5;
}
if ($result_list[2] == "Player B") {
    $market_id[] = 6;
}

if (strpos($result_list[3], "A : Straight") !== false || strpos($result_list[3], "A : Flush") !== false || strpos($result_list[3], "A : Trio") !== false || strpos($result_list[3], "A : Straight Flush") !== false) { 
    $market_id[] = 7;
}
if (strpos($result_list[3], "B : Straight") !== false || strpos($result_list[3], "B : Flush") !== false || strpos($result_list[3], "B : Trio") !== false || strpos($result_list[3], "B : Straight Flush") !== false) { 
    $market_id[] = 8;
}
...
while ($fetch_get_all_bet = mysqli_fetch_assoc($get_all_bet)) {
    ...
    $winner_flag=false;
   /*  if ((int)$bet_market_id == 7) {
		if ($result_list[2] != "No") {
			$winner_flag=true;
		}
	} */
    
    if ((int)$bet_market_id == 9) {
		if ($result_list[4] == "Yes" and $bet_type == "Back") {
			$winner_flag=true;
		}
        if ($result_list[4] != "Yes" and $bet_type == "Lay") {
			$winner_flag=true;
		}
	}else{
    ...
    if ($winner_flag) {
        ...
        if($bet_market_id == 7){
             if (strpos($result_list[3], "A : Straight Flush") !== false) { 
                ...
                $actual_win_amount2 = $actual_win_amount2 * 5;
            }
            else if (strpos($result_list[3], "A : Trio") !== false) { 
                $actual_win_amount2 = $actual_win_amount2 * 20;
            }
            else if (strpos($result_list[3], "A : Straight") !== false) { 
                $actual_win_amount2 = $actual_win_amount2 * 2;
            }
        }
        if($bet_market_id == 8){
