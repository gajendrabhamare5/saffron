...
$is_tied = is_tied($dragon_cards[0], $tiger_cards[0], $cards);
if ($is_tied) {
    $get_all_bet1 = $conn->query("select * from bet_teen_details where event_id='$mid' and bet_status=1 and event_type='$event_type'");
    while ($fetch_get_all_bet1 = mysqli_fetch_assoc($get_all_bet1)) {
        $bet_id = $fetch_get_all_bet1['bet_id'];
...
$is_pair = is_pair($dragon_cards[0], $tiger_cards[0], $cards);
...
$dragon_card_arr = $cards->$dragon_cards[0];
$bet_final_result .= '|'.$dragon_card_arr['name'].ucfirst($dragon_card_arr['type']);
...
$tiger_card_arr = $cards->$tiger_cards[0];
$bet_final_result .= '|'.$tiger_card_arr['name'].ucfirst($tiger_card_arr['type']);
...
function is_pair($dragon_cards, $tiger_cards, $cards) {
    $dragon_cards = $cards->$dragon_cards;
    $dragon_cards_rank = $dragon_cards['rank'];

    $tiger_cards = $cards->$tiger_cards;
...
function is_tied($dragon_cards, $tiger_cards, $cards) {
    /* $dragon_cards = $cards->$dragon_cards;
      $dragon_cards_rank = $dragon_cards['rank'];
