...
$remarkN=explode("#",$remark);
$remarkN=$remarkN[1];
if (strpos($remarkN, "P : Up") !== false) {
    $market_id[] = 3;
}
if (strpos($remarkN, "P : Down") !== false) {
    $market_id[] = 4;
}
if (strpos($remarkN, "D : Up") !== false) {
    $market_id[] = 5;
}
if (strpos($remarkN, "D : Down") !== false) {
    $market_id[] = 6;
}
...
/* if($row_count > 0){
  ...
  if (strpos($remark, "Tie") !== false) {
        $conn->query("update bet_teen_details set bet_status=2,bet_final_result='Tie' where event_id='$mid' and bet_status=1 and event_type='$event_type'");
...
while ($fetch_get_all_bet = mysqli_fetch_assoc($get_all_bet)) {
    ...
    if ($winner_flag) {
        ...
        if (strpos($remarkN, "P : Tie") !== false) {
            $bet_amount = $bet_amount / 2;
        } 
        if (strpos($remarkN, "D : Tie") !== false) {
            $bet_amount = $bet_amount / 2;
        }
