...
if(CASINO_MAINTENANCE == 1 and $user_id != 4){
	 $return = array(
        "status" => 'error',
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='lucky7'");
...
$user_status = $fetch_min_max_stake['Status'];
if ($user_status == 0) {
    $return = array(
        "status" => 'error',
        "message" => 'Your Account is Blocked.',
...
$bet_status = $fetch_min_max_stake['bet_status'];
...
$url = CASINO_DATA_URL ."?type=lucky7";
$ch = curl_init();
...
$api_event_id = str_replace("24.","",$api_event_id);
