...
$check_user_active_status = $conn->query("select Status,bet_status from user_master where Id=$user_id");
...
if ($dl_status == 0 || $mdl_status == 0 || $smdl_status == 0) {
    $return = array(
        "status" => 'error',
        "message" => 'Your Account is Blocked.',
    );
...
if ($dl_bet_status == 0 || $mdl_bet_status == 0 || $smdl_bet_status == 0) {
    $return = array(
        "status" => 'error',
        "message" => 'Your Bet is Blocked, Please contact your upline.',
    );
...
if ($type != 'No' and $type != 'Yes') {
    ...
    if($market_type == "MATCH_ODDS" && $odds > 4){
        $return = array(
            "status" => 'error',
            "message" => 'Bet Not Accept Rate Over 4.00 on Oneday and T20.',
        );
        save_rejection_log($conn,$user_id,$return['message'],$market_runner_name);
        echo json_encode($return);
        exit();        
    ...
    $min_stake_limit = $fetch_min_max_stake['min_stake'];
    $max_stake_limit = $fetch_min_max_stake['max_stake'];
    $fancy_min_stake_limit = $fetch_min_max_stake['min_fancy_stake'];
    $fancy_max_stake_limit = $fetch_min_max_stake['max_fancy_stake'];
            ...
            foreach ($data2 as $data22) {
                ...
                if ($market_name4 == "Match Odds" or $market_name4 == $market_type) {
                    ...
                    foreach ($t_runner_data as $runners_4_data) {
                        ...
                        if ($market_type == $market_name4 || $market_name4 == "Match Odds" || $market_type == "BOOKMAKER_ODDS") {
