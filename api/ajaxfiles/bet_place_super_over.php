...
if($marketId > 2){
	...
	if($market_type == "FANCY1_ODDS"){
	 ...
	 $sql_another_expo = $conn->query("select SUM(exposure_amount) as exposure_data from exposure_details where (event_id=$eventId and user_id=$user_id and market_type NOT LIKE '$current_array_market_type' AND market_id != $marketId ) OR (event_id != $eventId AND user_id=$user_id)");
	...
	$dd = get_current_bet_fancy_exposure_superopver($conn,$user_id,$eventId,$marketId,$current_bet);
