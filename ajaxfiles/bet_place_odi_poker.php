...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("ODI_POKER", $casino_lis_db)){
		$return = array(
		"status" => 'error',
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='poker';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='poker'");
...
$url = CASINO_DATA_URL ."?type=poker";
$ch = curl_init();
...
/* }else{
	$t3_datas = $api_data->t3;
	foreach($t3_datas as $t3_data){
		$api_sid = $t3_data->sid;
		if($api_sid == $marketId){
			$bet_status = $t3_data->gstatus;
			$back1 = $t3_data->b1;
			$lay1 = $t3_data->l1;
...
$api_event_id = str_replace("13.","",$api_event_id);
