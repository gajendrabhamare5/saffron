...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("2020_DRAGON_TIGER_LION", $casino_lis_db)){
		$return = array(
		"status" => 'error',
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='dtl20';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='dtl20'");
...
$url = CASINO_DATA_URL ."?type=dtl20";
$ch = curl_init();
...
$api_event_id = str_replace("32.","",$api_event_id);
...
$bet_market_name = "";
/* if($marketId == 1 or $marketId == 21 or $marketId == 41){
...
//echo "insert into bet_teen_details (`event_id`,`event_type`,`oddsmarketId`,`market_id`,`user_id`,`event_name`,`market_name`,`market_type`,`bet_type`,`bet_runs`,`bet_odds`,`bet_stack`,`bet_result`,`bet_margin_used`,`bet_time`,`bet_status`,`bet_win_amount`,`bet_ip_address`,`bet_user_agent`) values('$eventId','$eventType','$oddsmarketId','$marketId','$user_id','$eventName','$market_runner_name','$bet_market_name','$bet_type','00','$runs','$stack','0','$margin_used','$bet_time','1','$wiiningamount','$bet_ip_address','$bet_user_agent')";
