...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("BACCARAT", $casino_lis_db)){
		$return = array(
		"status" => 'error',
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='baccarat';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='baccarat'");
...
$url = CASINO_DATA_URL ."?type=baccarat";
$ch = curl_init();
...
foreach($t2_datas as $t2_data){
	...
	if($api_sid == $marketId){
		...
		if($bet_status != "1" or $bet_status != 1 and $bet_status != 'ACTIVE' and $bet_status != 'OPEN'){
			$return = array(
				"status"=>'error',
				"message"=>'Bet Suspended4',
			);
			save_rejection_log($conn,$user_id,($return['message']." #".__line__),$market_runner_name,$page_call_time,$get_data,$url);
			echo json_encode($return);
			exit();
...
$api_event_id = str_replace("37.","",$api_event_id);
...
if($type == "No" or $type == "Lay"){
	...
	$margin_used = $stack * ($runs);
	$bet_win_amount = $stack;
	$wiiningamount = $stack;
}else{
	$exposure_bet_type = "Back";
	$bet_win_amount = $stack * ($runs);
	$margin_used = $stack;
...
/* if($marketId == 1 or $marketId == 2 or $marketId == 3){
