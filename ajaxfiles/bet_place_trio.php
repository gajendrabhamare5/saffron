...
if (in_array("TRIO", $casino_lis_db)) {
    $return = array(
        "status" => 'error',
...
$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='trio'");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='trio'");
...
foreach ($t2_datas as $t2_data) {
    ...
    if ($api_sid == $marketId) {
        ...
        if(strtolower($market_runner_name) == "session"){
            if ($type == "Yes") {
                $api_rate =  $backs1;
            } else {
                $api_rate =  $lays1;
...
if(strtolower($market_runner_name) == "session"){
    if ($type == "No" or $type == "Lay") {
        $margin_used = ($stack * $odds) / 100;
        $bet_win_amount = $stack;
        $wiiningamount = $stack;
    } else {
        $bet_win_amount =  ($stack * $odds) / 100;
        $margin_used = $stack;
...
if ($odds != $api_rate and "$odds" != "$api_rate") {
    $return = array(
        "status" => 'error',
        "message" => "Invalid Rate.",
    );
