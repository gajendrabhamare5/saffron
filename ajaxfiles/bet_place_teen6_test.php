...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("TEEN6", $casino_lis_db)){
		$return = array(
		"status" => 'error',
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='teen6';");
...
$idssss = '1043113,1043115,1043116,1043117';
...
/* $get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='teen6'");
...
echo "<pre>";
print_r($final_net_exposure);
echo "</pre>";

echo "final_net_exposure= $final_net_exposure";
