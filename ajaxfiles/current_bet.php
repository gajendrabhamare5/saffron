...
if ($is_casino == 1) {
    $ttt = "SELECT count(*) totrecord,sum(bet_stack) as totamount FROM bet_teen_details where user_id=$user_id and bet_status='1' $query_condition $search  order by bet_time desc";
    $sql_data_all = $conn->query("SELECT count(*) totrecord,sum(bet_stack) as totamount FROM bet_teen_details where user_id=$user_id and bet_status='1' $query_condition $search order by bet_time desc");
    if ($length == "-1") {
        $sql_data = $conn->query("select * from bet_teen_details where user_id=$user_id and bet_status='1' $query_condition $search order by bet_time desc");
    } else {
        $sql_data = $conn->query("select * from bet_teen_details where user_id=$user_id and bet_status='1' $query_condition $search order by bet_time desc LIMIT $length OFFSET $start");
    }
} else {
    $sql_data_all = $conn->query("SELECT count(*) totrecord,sum(bet_stack) as totamount FROM bet_details where user_id=$user_id and bet_status='1' $query_condition $search order by bet_time desc");
    if ($length == "-1") {
        $sql_data = $conn->query("SELECT * FROM bet_details where user_id=$user_id and bet_status='1' $query_condition $search order by bet_time desc");
    } else {
        $sql_data = $conn->query("SELECT * FROM bet_details where user_id=$user_id and bet_status='1' $query_condition $search order by bet_time desc LIMIT $length OFFSET $start");
    }
}
