...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("DOLIDANA", $casino_lis_db)){
		$return = array(
		"status" => 'error',
		"message" => CASINO_MAINTENANCE_MSG,
	);
...
$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='dolidana';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='dolidana'");
...
$url = CASINO_DATA_URL ."?type=dolidana";
$ch = curl_init();
...
if($marketId == 1 || $marketId == 2){
	$api_event_id = $api_data->t1[0]->mainMid;
}
