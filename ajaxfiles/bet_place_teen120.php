...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("TEEN120", $casino_lis_db)){
		$return = array(
		"status" => 'error',
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='teen120';");
...
if($marketId == 1){
	$dataa=$conn->query("select * from bet_teen_details where event_id='$eventId' and market_id = '2'");
	if(mysqli_num_rows($dataa) > 0){
		$msgg="Bet not confirm bet already exists on Dealer.";
		 $return = array(
        "status" => 'error',
        "message" => $msgg,
		);
...
if($marketId == 2){
	$dataa=$conn->query("select * from bet_teen_details where event_id='$eventId' and market_id = '1'");
	if(mysqli_num_rows($dataa) > 0){
		$msgg="Bet not confirm bet already exists on Player.";
		 $return = array(
        "status" => 'error',
        "message" => $msgg,
		);
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='teen120'");
...
$url = CASINO_DATA_URL ."?type=teen120";
$ch = curl_init();
