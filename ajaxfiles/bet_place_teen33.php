...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("TEEN33", $casino_lis_db)){
		$return = array(
		"status" => 'error',
...
$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='teen33';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='teen33'");
...
$url = CASINO_DATA_URL ."?type=teen33";
$ch = curl_init();
...
$min_stack1 = $api_data->t2[0]->min;
$max_stack1 = $api_data->t2[0]->max;
 
$back2 = $api_data->t2[1]->b1;
$lay2 = $api_data->t2[1]->l1;
$min_stack2 = $api_data->t2[1]->min;
$max_stack2 = $api_data->t2[1]->max;
...
if($marketId == 1){
	if($min_stake_limit > $min_stack1){
		...
		if($min_stack1 > $stack){
			$return = array(
					"status"=>'error',
					"message"=>"Your Minimum Stake Limit is $min_stack1.",
				);
				save_rejection_log($conn,$user_id,($return['message']." #".__line__),$market_runner_name,$page_call_time,$get_data,$url);
			echo json_encode($return);
			exit();
	...
	if($max_stake_limit < $max_stack1){
		...
		if($max_stack1 < $stack){
			$return = array(
				"status"=>'error',
				"message"=>"Your Maximum Stake Limit is $max_stack1.",
			);
			save_rejection_log($conn,$user_id,($return['message']." #".__line__),$market_runner_name,$page_call_time,$get_data,$url);
			echo json_encode($return);
			exit();
	...
	if($min_stake_limit > $min_stack2){
		...
		if($min_stack2 > $stack){
			$return = array(
					"status"=>'error',
					"message"=>"Your Minimum Stake Limit is $min_stack2.",
				);
				save_rejection_log($conn,$user_id,($return['message']." #".__line__),$market_runner_name,$page_call_time,$get_data,$url);
			echo json_encode($return);
			exit();
	...
	if($max_stake_limit < $max_stack2){
		...
		if($max_stack2 < $stack){
			$return = array(
				"status"=>'error',
				"message"=>"Your Maximum Stake Limit is $max_stack2.",
			);
			save_rejection_log($conn,$user_id,($return['message']." #".__line__),$market_runner_name,$page_call_time,$get_data,$url);
			echo json_encode($return);
			exit();
