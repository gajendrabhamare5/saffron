...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("WORLI_MATKA", $casino_lis_db)){
		$return = array(
		"status" => 'error',
...
$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='worli';");
...
if (mysqli_num_rows($get_blockeventdata) > 0) {
	...
	$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='worli'");
...
$url = CASINO_DATA_URL ."?type=worli";
$ch = curl_init();
...
$api_event_id = str_replace("15.","",$api_event_id);
...
if($available_balance < $bet_total_amount){
	$return = array(
		"status"=>'error',
