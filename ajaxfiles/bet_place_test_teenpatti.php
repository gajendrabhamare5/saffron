...
foreach($get_data->odds->t2 as $market_data){
	...
	if(intval($market_data->sid) == intval($marketId)){
		$api_rate = $market_data->rate;
		$market_status = $market_data->gstatus;
	}
	...
	$nation2_name =  $market_data->nat;
}

if($market_status != 'True' and $bet_status != 1 and $bet_status != 'ACTIVE' and $bet_status != 'OPEN'){
	$return = array(
		"status"=>'error',
		"message"=>'Bet Suspended',
		"get_data"=>$get_data,
