...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("MOGAMBO", $casino_lis_db)){
		$return = array(
		"status" => 'error',
...
$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='mogambo';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='mogambo'");
...
$url = CASINO_DATA_URL ."?type=mogambo";
$ch = curl_init();
