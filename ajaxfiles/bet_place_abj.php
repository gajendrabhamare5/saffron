...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("AB20_2", $casino_lis_db)){
		$return = array(
		"status" => 'error',
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='abj';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='abj'");
...
$api_event_id = str_replace("38.","",$api_event_id);
...
if($marketId == 1 || $marketId == 2 || $marketId == 3){
	$market_runner_name = $market_runner_name.' A';
}

if($marketId == 4 || $marketId == 5 || $marketId == 6){
	$market_runner_name = $market_runner_name.' B';
}
