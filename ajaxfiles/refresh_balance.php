<?php
	...
	$exposure_balance = $fetch_exposure_balance['total_exposure']; */
		...
		$exposure = (float)($exposure_balance + $unmatched_exposure_balance);
	$return_array = array(
						"status"=>"ok",
						/* "balance"=>number_format($account_balance,2,'.',''), */
						"balance"=>(floor($account_balance) == $account_balance)
						...
						"exposure"=>(floor($exposure) == $exposure)
									? number_format($exposure, 0, '.', '')
