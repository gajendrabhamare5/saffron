...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("BTABLE2", $casino_lis_db)){
		$return = array(
		"status" => 'error',
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='btable2'");
...
$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='btable2';");
// $fetch_blockdata = mysqli_fetch_assoc($get_blockeventdata);
// echo "SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='goal'";
...
$url = CASINO_DATA_URL ."?type=btable2";
$ch = curl_init();
...
foreach($t2_datas as $t2_data){
	...
	if($api_sid == $marketId){
		...
		$back1 = $t2_data->b1;
		$lay1 = $t2_data->l1;
		if($bet_status != "ACTIVE" || $bet_status != "ACTIVE"){
			$return = array(
				"status"=>'error',
				"message"=>'Bet Suspended',
			);
...
$api_event_id = str_replace("26.","",$api_event_id);
...
/* else if($marketId == 7){
	...
	$bet_market_name = "DULHA_DULHAN";
}else if($marketId == 15){
	$bet_market_name = "DULHA_DULHAN_1";
}else if($marketId == 8){
	$bet_market_name = "RED_BLACK";
}else if($marketId == 9){
	$bet_market_name = "RED_BLACK_1";
}else if($marketId == 13){
	$bet_market_name = "CARD_A";
}else if($marketId == 10){
	$bet_market_name = "CARD_J";
}else if($marketId == 11){
	$bet_market_name = "CARD_Q";
}else if($marketId == 12){
	$bet_market_name = "CARD_K";
} */
