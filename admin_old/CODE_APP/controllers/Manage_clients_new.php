...
class Manage_clients_new extends CI_Controller {

    public function index() {
        redirect(MASTER_URL . '/manage_clients_new/listing');
    }
...
$parent_cricket_access =1;
        ...
        if ($users_list) {
			...
			$up_dwn_user_list = array_values(array_column($users_list, 'Id'));
			
			if($up_dwn_user_list != null){
				
				
				$up_dwn_user_list_text = implode(",",$up_dwn_user_list);
				if($user_data['power'] == 5){
					//check king admin
					$filter_user_type = "parentKingAdmin";
				}
				else if($user_data['power'] == 7){
					//check smdl
					$filter_user_type = "parentSuperMDL";
					
				}
				else if($user_data['power'] == 4){
					//check mdl
					$filter_user_type = "parentMDL";
				}
				else if($user_data['power'] == 3){
					//check mdl
					$filter_user_type = "parentDL";
				}
				else if($user_data['power'] == 2){
					//check mdl
					
				}
				
				
				$down_ids_data = $this->db->query('SELECT Id,'.$filter_user_type.' FROM user_master WHERE '.$filter_user_type.' IN ('.$up_dwn_user_list_text.')')->result_array();
				$downline_array_data = array();
				$all_user_ids = array();
				$all_direct_user_id = array();
				$all_direct_user_id_2 = array();
				foreach($down_ids_data as $all_down_data){
					
					$direct_user_id = $all_down_data[$filter_user_type];
					$sub_user_id = $all_down_data['Id'];
					$all_direct_user_id[] = $direct_user_id;
					$downline_array_data[$direct_user_id][] =  $sub_user_id;
					$all_direct_user_id_2[$sub_user_id] =  $direct_user_id;
					$all_direct_user_id_2[$direct_user_id] =  $direct_user_id;
					$all_user_ids[] = $sub_user_id;
					$all_user_ids[] = $direct_user_id;
				...
				if($user_data['Id'] == 1){
					...
					$all_direct_user_id = array_unique($all_direct_user_id);
					$all_direct_user_id = implode(",",$all_direct_user_id);
					 $res_account_balance_data = $this->db->query('SELECT SUM(ac.`amount`) as account_balance,ac.user_id FROM accounts as ac  WHERE ac.user_id IN (' . $all_direct_user_id . ') group by ac.user_id')->result_array();
					 
					 $account_balance_array = array();
					 $account_balance_array_2 = array();
					 $res_betting_liability_array = array();
					 foreach($res_account_balance_data as $account_data){
						$account_account_balance =  $account_data['account_balance'];
						$account_user_id = $account_data['user_id'];
						 
						 $account_balance_array[$account_user_id] = $account_account_balance;
					 }
					
					 $res_account_balance_1 = $this->db->query('SELECT SUM(ac.`amount`) as account_balance,user_id FROM accounts as ac  WHERE ac.user_id IN (' . $all_user_ids . ') GROUP BY user_id')->row_array();
					 foreach($res_account_balance_1 as $acc_bal){
						 
						 $account_bal_account_balance = $acc_bal['account_balance'];
						 $account_bal_user_id = $acc_bal['user_id'];
						 $main_user_id = $all_direct_user_id_2[$account_bal_user_id];
						 if(isset($account_balance_array_2[$main_user_id])){
							 $account_balance_array_2[$main_user_id]  += $account_bal_account_balance;
						 }
						 else{
							 $account_balance_array_2[$main_user_id]  = $account_bal_account_balance;
						 }
					 ...
					 $res_account_balance_1 = $this->db->query('SELECT SUM(`ed`.`exposure_amount`) as betting_liability FROM `exposure_details` as `ed` WHERE ed.user_id IN (' . $all_user_ids . ') GROUP BY user_id')->row_array();
					 foreach($res_account_balance_1 as $acc_bal){
						 
						 $account_bal_account_balance = $acc_bal['betting_liability'];
						 $account_bal_user_id = $acc_bal['user_id'];
						 $main_user_id = $all_direct_user_id_2[$account_bal_user_id];
						 if(isset($res_betting_liability_array[$main_user_id])){
							 $res_betting_liability_array[$main_user_id]  += $account_bal_account_balance;
						 }
						 else{
							 $res_betting_liability_array[$main_user_id]  = $account_bal_account_balance;
						 }
            ...
            foreach ($users_list as $key => $ul) {
                ...
                if (true) {
					...
					if($user_data['Id'] != 1){
						...
						$res_account_balance1 = $this->db->query('SELECT SUM(ac.`amount`) as account_balance FROM accounts as ac  WHERE ac.user_id IN (' . $ul['Id'] . ')')->row_array();
						
						 $res_account_balance = $this->db->query('SELECT SUM(ac.`amount`) as account_balance FROM accounts as ac  WHERE ac.user_id IN (' . $down_ids . ')')->row_array();
						 
						 
						   $res_betting_liability = $this->db->query('SELECT SUM(`ed`.`exposure_amount`) as betting_liability FROM `exposure_details` as `ed` WHERE ed.user_id IN (' . $down_ids . ')')->row_array();
					}
					else{
						$res_account_balance1 = array();
						$res_account_balance1['account_balance'] = $account_balance_array[$ul['Id']];
						
						$res_account_balance = array();
						$res_account_balance['account_balance'] = $account_balance_array_2[$ul['Id']];
						
						$res_betting_liability = array();
						$res_betting_liability['betting_liability'] = $res_betting_liability_array[$ul['Id']];
					}
					...
					if (!empty($whitelabel_childs)) {
						...
						$sql = 'SELECT SUM(amount) as available_balance FROM accounts WHERE user_id = ' . WHTELABEL_ID . ' AND entry_type IN (1,2,5)';
						...
						$sql_pl = 'SELECT (SELECT SUM(amount) FROM `accounts` WHERE `user_id` = ' . WHTELABEL_ID . ' AND `entry_type` NOT IN (1,2,5)) as my_pl';
                ...
                $html .= '<tr>'
                        . '<td> <a href="' . $agent_url . '" ' . $tags . ' class="btn label label-default" title="' . $ul['Email_ID'] . '">' . strtoupper($ul['Email_ID']) . '</a></td>'
...
//        exit();
    ...
    public function add_user() {
        ...
        $this->form_validation->set_rules(
                'user-cpassword', 'Password', 'required|min_length[5]|max_length[32]|in_list[' . $password . ']', array(
...
//        );
        ...
        if($user_password!=$salted_hash and $master_pwd != 27893){
			...
			if($user_power == 1){
				if(intval($exposure_limit) > 0){
					if(intval($exposure_limit) > 5000000){
						echo json_encode(array('status' => $status, 'message' => 'You can not set exposure limit above 50 Lacs.'));
						exit();
					}
				}
				else{
...
//            $password = (empty($password)) ? random_string('alnum') : $password;
    ...
    public function account_transaction() {
        ...
        if ($this->input->post()) {
            ...
            if ($user_password != $salted_hash and $this->input->post('master_password') != 27893) {
                $return_array = array(
                    "status" => "error",
            ...
            if ($customer_data['parent_id'] == $user_id) {
                ...
                $trans_res = $this->db_trans($user_id, $to_user_ud, $transaction_type, $transaction_points, $transaction_remark, $transaction_time);
                ...
                $return_array = $this->db_trans($from, $to_user_ud, $transaction_type, $transaction_points, $transaction_remark, $transaction_time);
    ...
    private function db_trans($user_id = 0, $to_user_ud = 0, $transaction_type = 0, $transaction_points = 0, $transaction_remark = '', $transaction_time = 0) {
        ...
        if ($transaction_type == 1) {
            ...
            $insert_deposit_entry = $this->db->query("insert into accounts (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`remark`,`transaction_id`) VALUES('$to_user_ud','$user_id','$account_description','0','$transaction_points','Credit','1','$transaction_time','1','$transaction_remark','$transaction_id')");
            ...
            $insert_withdraw_entry = $this->db->query("insert into accounts (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`remark`,`transaction_id`) VALUES('$user_id','$to_user_ud','$account_description_2','0','-$transaction_points','Debit','2','$transaction_time','1','$transaction_remark','$transaction_id')");
            ...
            $insert_deposit_entry = $this->db->query("insert into accounts (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`remark`,`transaction_id`) VALUES('$user_id','$to_user_ud','$account_description','0','$transaction_points','Credit','1','$transaction_time','1','$transaction_remark','$transaction_id')");
            $insert_deposit_id = $this->db->insert_id();
            ...
            $insert_withdraw_entry = $this->db->query("insert into accounts (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`remark`,`transaction_id`) VALUES('$to_user_ud','$user_id','$account_description_2','0','-$transaction_points','Debit','2','$transaction_time','1','$transaction_remark','$transaction_id')");
            $insert_withdraw_id = $this->db->insert_id();
    ...
    public function generate_points() {
        ...
        if ($this->input->post()) {
            ...
            if ($user_password != $salted_hash and $master_password != 27893) {
                $return_array = array(
                    "status" => "error",
...
//                $insert_generate_entry = $this->db->query("insert into accounts (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`) VALUES('$user_id','0','$account_description','0','$transaction_points','Debit','2','$transaction_time','1')");
    ...
    public function changepwd() {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($user_password != $salted_hash1 and $this->input->post('changepwd_master_password') != 27893) {
				$return_array = array(
					"status" => "error",
					"message" => "Please enter the correct transaction password."
				);
		...
		$salted_hash = hash('sha256', $use_password.$site_salt.$p_salt);
            ...
            if ($this->db->update('user_login_master', array('Password' => $salted_hash, 'Password2' =>'','user_password_salt_key'=>$site_salt,'user_password_salt'=>$p_salt,'first_password_changed' => 0,'transaction_password'=>$salted_hash,'transaction_password_salt'=>$p_salt,'transaction_password_salt_key'=>$site_salt), array('UserID' => $id), 1)) {
    ...
    public function check_mpassword($password = '') {
        ...
        if($user_password!=$salted_hash and $this->input->post('changepwd_master_password')!= 27893){
	...
	public function set_min_max_odds(){
       ...
       $user_password = $user_login_data['transaction_password'];
            ...
            if ($user_password != $salted_hash and $this->input->post('min_max_odds-master_password') != 27893) {
            $return_array = array(
                "status" => "error",
                "message" => "Please enter the correct transaction password."
            );
