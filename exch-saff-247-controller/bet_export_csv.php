<?php
	...
	/* if(empty($master_name_new) && empty($client_name_new)){
		$user_id_array=array();
		$user_data=$conn->query("select u1.UserID,u1.Email_ID,u2.Email_ID as master_name from user_login_master u1 inner join  user_login_master u2 on u2.UserID=u1.parentDL where u1.UserType=1");
	...
	{
		$master_name_new1=implode("','",$master_name_new);
		$user_data_master_all=$conn->query("select UserID,Email_ID from user_login_master where UserType IN (2,3,4) and Email_ID IN ('$master_name_new1')");
	}
	else{ */
	...
	echo "select * from `bet_details` where 1=1 $where"; */
	 header("Content-Type:text/csv; charset=utf-8");
		header("Content-Disposition:attectment;filename=export_bet.csv");
		$output=fopen("php://output","w");
		if(!empty($status_name) && $status_name=='hard_delete'){
			fputcsv($output,array('Bet Id','User Name','Master Name','Event Name','Market Name','Market Type','Bet Type','Run','Odds', 'Stack','Result','Status','Time','IP Address','User Agent','Delete Time','Delete IP Address'));
			$q1=$conn->query("select * from `bet_details_deleted` where 1=1 $where");
		}else{
			fputcsv($output,array('Bet Id','User Name','Master Name','Event Name','Market Name','Market Type','Bet Type','Run','Odds', 'Stack','Result','Status','Time','IP Address','User Agent'));
			$q1=$conn->query("select * from `bet_details` where 1=1 $where");
		}
		...
		foreach($q1 as $i1)
		{
			...
			$market_type1=$i1['market_type'];
			if(strtolower($market_type1)=="fancy_odds"){
				$market_type="FANCY";
			}
			if(strtolower($market_type1)=="match_odds"){
				$market_type="MATCH";
			}
			if(strtolower($market_type1)=="bookmaker_odds"){
			...
			if(!empty($status_name) && $status_name=='hard_delete'){
				...
				fputcsv($output,array($bet_id,$user_name,$master_name,$event_name,$market_name,$market_type,$bet_type,$bet_runs,$bet_odds,$bet_stack,$bet_result,$bet_status,$bet_time,$bet_ip_address,$bet_user_agent,$deleted_time,$deleted_ip_address)); 
			}else{
				fputcsv($output,array($bet_id,$user_name,$master_name,$event_name,$market_name,$market_type,$bet_type,$bet_runs,$bet_odds,$bet_stack,$bet_result,$bet_status,$bet_time,$bet_ip_address,$bet_user_agent)); 
			}
		}		
		fclose($output);  
		/* 
