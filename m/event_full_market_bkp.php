...
/*
$sql_market_limit = $conn->query("select * from event_market_limit where event_id=$event_id AND status = 0");
...
$match_max = $bookmaker_min = $bookmaker_max = 1;
...
$get_bet_processing_second = $conn->query("select match_odds_processing,fancy_odds_processing from  user_master where Id=$parentSuperMDL");
$fetch_get_bet_processing_second = mysqli_fetch_assoc($get_bet_processing_second);
$match_odds_processing = $fetch_get_bet_processing_second['match_odds_processing'];
$fancy_odds_processing = $fetch_get_bet_processing_second['fancy_odds_processing'];
if($match_odds_processing == ""){
	$match_odds_processing = 0;
}
if($fancy_odds_processing == ""){
	$fancy_odds_processing = 0;
}
$match_odds_processing_sec = $match_odds_processing * 1000;
$fancy_odds_processing_sec = $fancy_odds_processing * 1000;
                ...
                <span id="count_open_bet">0</span>  )</a></li>
                    ...
                    <div class="tab-content">
                        <div id="odds" class="tab-pane active">
							...
							<?php }?>
                            <!---->
                            <?php if($event_id != ELECTION_EVENT_ID){ ?>
                            	<div id="match_odds_all_full_markets"></div>
                            <?php } ?>
                            <div id="bookmaker_market_div_secure"> </div>
                            <!---->
                            <?php if($eventType == 4){ ?>
								<div class="">
...
//                                        $("#lay_1_"+selectionId+"_"+market_marketid).removeClass('rate_change_link');
                        ...
                        if(args.body.khado[0].value){
                            ...
                            if(!n1 && !n2 && !n3){
                                    ...
                                    if(m1){
                                            ...
                                            back_attribute = " event_name='"+event_name+"' fullmarketodds='"+runsYes+"' side='Lay' marketid='"+marketId+"' eventid='"+eventId+"'  selectionid='"+marketId+"' market_odd_name='KHADO_ODDS' runner='"+marketName+"' marketname='"+marketName+"' markettype='FANCY_ODDS' inplay='1' fullfancymarketrate='"+oddsYes+"' winnig_zone='"+winning_zone+"'";

                                            lay_attribute = " event_name='"+event_name+"' fullmarketodds='"+runsNo+"' side='Lay' marketid='"+marketId+"' eventid='"+eventId+"' selectionid='"+marketId+"' market_odd_name='KHADO_ODDS' runner='"+marketName+"' marketname='"+marketName+"' markettype='FANCY_ODDS' inplay='1' fullfancymarketrate='"+oddsNo+"'";
    ...
    jQuery(document).on("input", "#inputStake", function () {
        ...
        if(bet_markettype == "BOOKMAKER_ODDS"){
            odds = (odds/100)+1;
        }
    ...
    jQuery(document).on("click", "#placeBet", function () {
             ...
             $("#placeBet").addClass('disabled');
                    ...
                    if(bet_markettype != "FANCY_ODDS"){
                        ...
                        bet_seconds = <?php echo $match_odds_processing; ?>;
                        bet_sec = <?php echo $match_odds_processing_sec; ?>;
                    }else{
                        ...
                        bet_seconds = <?php echo $fancy_odds_processing; ?>;
                        bet_sec = <?php echo $fancy_odds_processing_sec; ?>;
                    }
