...
$conn->query("insert into common_wallet_response set `response`='$raw_json',`wallet_session`='$wallet_session',`pass_key`='$passkey',`request_keys`='$r_columns',`server_keys`='$s_columns]',`server_values`='$s_values',`request_values`='$r_values',`page_name`='transaction',`datetime`='$datetime'");
...
if ($fetch_userdata['Status'] == 0 || $fetch_dldata['Status'] == 0 || $fetch_mdldata['Status'] == 0 || $fetch_smdldata['Status'] == 0 || $fetch_kingdata['Status'] == 0) {
	...
	if (strtolower($txnType) == "credit" || isset($_REQUEST['rollback'])) {
		http_response_code(400);
...
if ($wallet_session != $db_wallet_session && strtolower($txnType) == "debit") {
	http_response_code(400);
	$return = array(
		"code" => 'INVALID_TOKEN',
...
if ($user_balance < $amount && strtolower($txnType) == "debit") {
	http_response_code(400);
	$return = array(
...
if (strtolower($txnType) == "debit") {
	...
	$account_description = "#Loss GAME ID $gameId Round ID $roundId Transaction ID $txnId at $transaction_time2";


	if (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $amount != 0)) {
		$insert_user_account = $conn->query("insert into accounts (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`event_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`game_type`,`transaction_id`,remark) VALUES('$bet_user_id','0','$account_description','-1','$txnId','-$amount','Debit','7','$transaction_time','1',1,'$transaction_id','$account_description')");
	}
	if (INSERT_ACCOUNTS_TEMP && (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $amount != 0))) {
		$conn->query("insert into accounts_temp (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`event_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`game_type`,`transaction_id`,remark) VALUES('$bet_user_id','0','$account_description','-1','$txnId','-$amount','Debit','7','$transaction_time','1',1,'$transaction_id','$account_description')");
	}
...
if (strtolower($txnType) == "credit") {

	$fetch_bet = $conn->query("select * from accounts where event_id='$betId'");
	if (mysqli_num_rows($fetch_bet) <= 0) {
		http_response_code(400);
		$return = array(
			"code" => 'REQUEST_DECLINED',
			"message" => 'Bet not avilable',
		);
		echo json_encode($return);
		exit();
	}
	...
	$transaction_id = $txnId . '-' . $bet_user_id;
	$account_description = "#$lable GAME ID $gameId Round ID $roundId Transaction ID $txnId at $transaction_time2";
	if (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $amount != 0)) {
...
if (isset($_REQUEST['rollback']) && !isset($data->txnType)) {
	$lable = "Rollover";
	$lable1 = "Rollover";
	$fetch_bet = $conn->query("select * from accounts where event_id='$betId'");
	if (mysqli_num_rows($fetch_bet) <= 0) {
