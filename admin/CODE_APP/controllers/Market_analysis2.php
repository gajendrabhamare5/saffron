...
class Market_analysis2 extends CI_Controller
{
	...
	public function index()
	{
		...
		$this->load->view('market-analysis/list2', array('handler' => $this));
	...
	public function get_analysis()
	{
		...
		$filter_user_id = $this->input->post('uid');
		if (!empty($filter_user_id)) {

			$get_user = $this->db->query("
				SELECT Id 
				FROM user_master 
				WHERE email_id = '".$filter_user_id."'
			")->row_array();

			if (!empty($get_user)) {
			
				$filter_user_id = $get_user['Id'];
			} 
		}
...
if (!empty($filter_user_id)) {
    ...
    // ONLY SELECT THIS USER
    $where_event = " AND b.user_id = ".$filter_user_id;
    $is_filter_user = true;
		...
		$get_all_fancy_active_user_list = $this->db->query("select u.UserID from user_login_master as u where 1=1 $where_event1")->result_array();
		$user_idss = array_values(array_column($get_all_fancy_active_user_list, 'UserID'));
		if ($user_data['power'] < 5 or $user_data['power'] == 7) {
			$user_list = implode(',', $user_idss);
			if (empty($user_idss)) {
				$where_event = ' AND b.user_id IN (-1)'; // Avoid using `IN()` with an empty list
			} else {
				$where_event = " AND b.user_id IN ($user_list)";
			}
		}
		...
		foreach ($get_all_fancy_active_user_list123 as $downline_user) {
			...
			$percentage_parentDL = isset($master_user_data[$parentDL]) ? $master_user_data[$parentDL] : 0;
			$percentage_parentMDL = isset($master_user_data[$parentMDL]) ? $master_user_data[$parentMDL] : 0;
			$percentage_parentSuperMDL = isset($master_user_data[$parentSuperMDL]) ? $master_user_data[$parentSuperMDL] : 0;
			$percentage_parentKingAdmin = isset($master_user_data[$parentKingAdmin]) ? $master_user_data[$parentKingAdmin] : 0;
		...
		/* echo "</br>";
		  ...
		  $where_filter_user = '';
			...
			if (!empty($filter_user_id)) {
				$where_filter_user = " AND b.user_id = ".$filter_user_id;
			}
		...
		$sql_event_id = "
		  ...
		  WHERE b.bet_status = 1 $where_event  $where_filter_user
		  GROUP BY b.user_id, b.event_id , b.market_id, b.bet_type, b.market_type
		  ";
		...
		foreach ($event_ids_data as $event_id_data) {
			if (!isset($events_info[$event_id_data['event_id']])) {
				$event_id_data_new = array(
					"event_id" => $event_id_data['event_id'],
					"event_name" => $event_id_data['event_name'],
					"oddsmarketId" => $event_id_data['oddsmarketId'],
					"event_type" => $event_id_data['event_type'],
				);
				$events_info[$event_id_data['event_id']] = $event_id_data_new;
			}
			if (!in_array($event_id_data['event_id'], $event_ids)) {
				$event_ids[] = $event_id_data['event_id'];
			}

			$bets_by_events_arr[$event_id_data['event_id']][] = $event_id_data;
			$event_markets[$event_id_data['event_id']][] = $event_id_data;
		}
		...
		$sql = 'SELECT e.* FROM  event_market_id e WHERE e.event_id IN (' . $event_ids_str . ') GROUP BY e.event_id , e.market_id';
		...
		foreach ($event_markets as $key => $event_market) {
			...
			echo "</pre>";
				...
				foreach ($bets_data as $bet_data) {
                    ...
                    $user_share = isset($self_percentage_array[$bet_data['user_id']]) ? $self_percentage_array[$bet_data['user_id']] : 0;
                     $mid = $bet_data['market_id'];
					/* $uni_key = strval($event_id) . "_" . strval($mid); */
				$uni_key = strval($mid);
                    $win_contribution = ($bet_data['total_win_amount'] * $user_share) / 100;
                    $margin_contribution = ($bet_data['total_margin_used'] * $user_share) / 100;
                    
                    if ($bet_data['bet_type'] == 'Back') {

                        $back_win_sum[$uni_key] = ($back_win_sum[$uni_key]) + $win_contribution;
                        $back_margin_sum[$uni_key] = ($back_margin_sum[$uni_key]) + $margin_contribution;

                        $global_back_margin += $margin_contribution;

                    } else { // Lay

                        $lay_win_sum[$uni_key] = ($lay_win_sum[$uni_key]) + $win_contribution;
                        $lay_margin_sum[$uni_key] = ($lay_margin_sum[$uni_key]) + $margin_contribution;
                ...
                foreach ($event_market as $market_data) {
                    ...
                    $market_id = $market_data['market_id'];
						$mid = $market_data['market_id'];
                    $market_name = $market_data['market_name'];
					/* $uni_key = strval($event_id) . "_" . strval($mid); */
				$uni_key = strval($mid);

                   /*  $bw = $back_win_sum[$market_id];
                    $bm = $back_margin_sum[$market_id];
                    $lw = $lay_win_sum[$market_id];
                    $lm = $lay_margin_sum[$market_id]; */
					$bw = 0;
				if (isset($back_win_sum[$uni_key])) {
					$bw = $back_win_sum[$uni_key];
				}

				$bm = 0;
				if (isset($back_margin_sum[$uni_key])) {
					$bm = $back_margin_sum[$uni_key];
				}

				$lw = 0;
				if (isset($lay_win_sum[$uni_key])) {
					$lw = $lay_win_sum[$uni_key];
				}

				$lm = 0;
				if (isset($lay_margin_sum[$uni_key])) {
					$lm = $lay_margin_sum[$uni_key];
				}
	...
	public function get_analysis_old()
	{
		/* echo "</br>";
	  	echo "1";
	  	echo "</br>";
		...
		if ($user_data['power'] < 5 or $user_data['power'] == 7) {
			$user_list = implode(',', $user_idss);
			if (empty($user_idss)) {
				$where_event = ''; // Avoid using `IN()` with an empty list
			} else {
				$where_event = " AND b.user_id IN ($user_list)";
			}
		}
		...
		$sql_event_id = "
		  ...
		  WHERE b.bet_status = 1 $where_event
		  GROUP BY b.user_id, b.event_id , b.market_id, b.bet_type
		  ";
		$event_ids_data = $this->db->query($sql_event_id)->result_array();

		/* $sql_event_id = "SELECT b.event_id, b.event_name, b.oddsmarketId, b.event_type FROM bet_details b WHERE b.bet_status = 1 $where_event GROUP BY b.event_id";
		...
		$sql = 'SELECT e.* FROM  event_market_id e WHERE e.event_id IN (' . $event_ids_str . ') AND (e.market_type LIKE "MATCH_ODDS" or e.market_type LIKE "BOOKMAKER_ODDS") GROUP BY e.event_id , e.market_id';
		...
		/* $sql_bet = '
					...
					WHERE b.event_id IN (' . $event_ids_str . ') 
						AND b.bet_status = 1
						AND (b.market_type LIKE "MATCH_ODDS" or b.market_type LIKE "BOOKMAKER_ODDS")
