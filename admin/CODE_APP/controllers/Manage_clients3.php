...
class Manage_clients3 extends CI_Controller
{


    public function index()
    {
        redirect(MASTER_URL . '/manage_clients3/listing3');
    }


    public function listing3($agent_id = '')
    {
        ...
        $this->load->view('manage-clients/list_work2', array('handler' => $this));
    ...
    public function get_user_list($offset = '', $user_status = '1', $user2 = 'hide')
    {
        ...
        if ($users_list) {
            ...
            if ($user2 == 'show') {
                $this->load->library('hisab');
                $hisab_ctx = $this->hisab->build_hisab_context($users_list, true);
            }
            ...
            foreach ($users_list as $key => $ul) {
                ...
                $account_balance = $account_balance1 = $client_pl = $exposure = $avl_balance = 0;
                $default_partnership = $ul['my_percentage'];

                if ($user2 == 'show') {
                    $row_hisab = $this->hisab->compute_row_hisab($ul, $hisab_ctx);
                    $account_balance = $row_hisab['account_balance'];
                    $account_balance1 = $row_hisab['account_balance1'];
                    $client_pl = $row_hisab['client_pl'];
                    $exposure = $row_hisab['exposure'];
                    $avl_balance = $row_hisab['avl_balance'];
                ...
                if ($user2 == 'hide') {
                    $agent_url = ($ul['power'] == 1) ? 'javascript:void(0);' : '' . MASTER_URL . '/manage-clients3/listing/' . $user_id;
                } else {
                    $agent_url = ($ul['power'] == 1) ? 'javascript:void(0);' : '' . MASTER_URL . '/manage-clients3/listing2/' . $user_id;
                }
    ...
    private function get_users($limit = '', $offset = '', $user_status = '1')
    {
        ...
        if (!empty($limit)) {
            ...
            $this->db->select('u.Id,u.Name,u.Email_ID,u.power,u.credit_reference,u.net_exposure_limit,u.my_percentage,u.parent_id,u.Status,bet_status,u.cricket_access,u.soccer_access,u.tennis_access,u.video_access');
        } else {
