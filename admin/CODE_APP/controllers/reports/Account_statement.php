...
class Account_statement extends CI_Controller
{
    ...
    function get_account_statement_data($type = 'data')
    {
        ...
        if ($report_type) {
            if ($report_type == 2){
                ...
                $sql_where .= ' AND ac.bet_id != 0 AND game_type = 0';
            }
           else if ($report_type == 5){

                $sql_where .= ' AND ac.bet_id != 0 AND game_type = 1';
        ...
        if ($game_name) {
            ...
            if ($report_type > 2 && $game_name != 'all') {
                if ($type == 'data')
        ...
        if ($type == 'data') {
            ...
            $sql_select .= ', IF(ab1.market_name IS NULL,ab2.market_name, IF(ab1.winner_name = "", ab1.market_name,ab1.winner_name)) as market_name';
            ...
            $sql_select .= ', IF(ac.game_type = 0,IF(ab1.market_type = "FANCY_ODDS" OR ab1.market_type = "METER_ODDS" OR ab1.market_type = "KHADO_ODDS" OR ab1.market_type = "EVEN_ODD" OR ab1.market_type = "ODD_EVEN", ab1.market_id, ab1.market_type),ab2.event_type) as mgp_id';
        }
        ...
        if ($type == 'data') {
            ...
            $this->db->join('bet_details ab1', 'ab1.bet_id = ac.bet_id AND 0 = ac.game_type', 'LEFT');
            $this->db->join('bet_teen_details ab2', 'ab2.bet_id = ac.bet_id AND 1 = ac.game_type', 'LEFT');
    ...
    public function get_bets()
    {
        ...
        if ($game_type == 0)
            $bets_data = $this->db->get('bet_details as b')->result_array();
        else
            $bets_data = $this->db->get('bet_teen_details as b')->result_array();
