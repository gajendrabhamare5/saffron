...
class Current_bets extends CI_Controller {
    ...
    private function get_current_bets_data() {
        ...
        if ($this->input->post('sport_type') != "sport") {
            return array();
        }
        ...
        if ($backlay = $this->input->post('backlay', TRUE)) {
            if($backlay != "all"){
                $this->db->where('b.bet_type = "' . ucwords($backlay) . '"');
            }
        }
	...
	private function get_current_bets_data_casino() {
        ...
        if ($this->input->post('sport_type') != "casino") {
            return array();
        }
