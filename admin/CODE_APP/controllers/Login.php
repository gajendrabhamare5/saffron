...
class login extends CI_Controller
{
	...
	public function index($mode = 'standard')
	{
		...
		if (!$this->input->is_ajax_request()) {
					...
					if (true) {
						...
						$whereArr = array('Email_ID' => $email);
							...
							if ($user_query->num_rows() > 0) {
								...
								/* if($user_password_salt == "" && $user_password_salt_key == ""){
									...
									$this->db->query("update user_login_master set user_password_salt='$p_salt',user_password_salt_key='$site_salt',Password='$salted_hash' where UserID=1");
									$this->layout['alerts']['error'][] = 'The username or password is incorrect 123';
						...
						*/
								...
								if ($user_password == $salted_hash) {
									...
									if ($User_master_data['Status'] == 1 && $user_data['UserType'] > 1) {
										...
										$auth_user_verification_status = $User_master_data['user_verification_status'];
										$auth_user_verification_type = $User_master_data['user_verification_type'];
										...
										if ($auth_user_verification_status == "ENABLED") {
											if ($auth_user_verification_type == "Telegram") {
												...
												$err = curl_error($curl);
												if (curl_errno($curl)) {
													$this->layout['alerts']['error'][] = curl_error($curl);
													return;
												}
												curl_close($curl);
												$response = (array) json_decode($response);
												if (!$response['status']) {
													$this->layout['alerts']['error'][] = $response['message'];
													return;
												}
											}
											$this->session->set_userdata('CLIENT_AUTH_STATUS', true);
											$this->session->set_userdata('CLIENT_AUTH_UID', $user_data['UserID']);
											header("Location: " . MASTER_URL . "/LoginAuth");
											return;
										}
										$this->session->set_userdata('user_login_data', $user_data);
			...
			$this->load->database();
			$logo_data = $this->db->get_where('logo', ['Id' => 1])->row_array();
			$this->load->view('login', array('handler' => $this,'logo_data' => $logo_data));
		} else {
