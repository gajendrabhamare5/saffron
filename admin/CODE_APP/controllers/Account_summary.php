...
class Account_summary extends CI_Controller {
    ...
    public function index() {
        ...
        if(false){
			...
			$sql = 'SELECT SUM(amount) as available_balance FROM '.ACCOUNT_DATABASE_NAME.' WHERE user_id = ' . $user_id . ' AND entry_type IN (1,2,5) AND status = 1';
			$res = $this->db->query($sql)->row();
			$available_balance = $res->available_balance;
			...
			$sql_pl = 'SELECT (SELECT SUM(amount) FROM '.ACCOUNT_DATABASE_NAME.' WHERE `user_id` = ' . $user_id . ' AND `entry_type` NOT IN (1,2,5) AND status = 1) as my_pl';
			$pl_data = $this->db->query($sql_pl)->row();
        ...
        $down_pl = $this->hisab->sum_direct_children_pl($user_id);
        $down_level_pl = $down_pl;
