...
class Manage_clients extends CI_Controller
{
    ...
    public function listing2($agent_id = '')
    {
        ...
        $this->load->view('manage-clients/list_work', array('handler' => $this));
        $this->load->view('footer', array('handler' => $this));
    }

    public function get_user_list($offset = '', $user_status = '1', $user2 = 'hide')
    {
        ...
        $users_list = $this->get_users($limit, $offset, $user_status);
        ...
        if ($users_list) {
            ...
            ")->result_array();
                ...
                foreach ($down_users_data as $du) {
                    foreach (['parentDL', 'parentMDL', 'parentSuperMDL', 'parentKingAdmin'] as $p) {
                        if (!empty($du[$p]) && in_array($du[$p], $user_ids)) {
                            $parent_children_map[$du[$p]][] = $du['Id'];
                        }
                    }
                }

                $all_ids = array_unique(array_merge($user_ids, array_column($down_users_data, 'Id')));
                ...
                WHERE user_id IN ($all_ids_str)
                GROUP BY user_id
            ")->result_array();
                $balances_map = array_column($balances, 'account_balance', 'user_id');
            ...
            ")->result_array();
                ...
                foreach ($liabilities as $li) {
                    $liabilities_map[$li['user_id']] = [
                        'betting_liability' => $li['betting_liability'],
                        'master_betting_liability' => $li['master_betting_liability']
                    ];
                }

                $account_balance_whitelabel = 0;
                if ($user_data['power'] == 6 && !$this->input->post('agent_id', TRUE)) {

                    $res_whitelabel_childs = $this->db->query(
                        "
                    SELECT Id FROM user_master WHERE parent_id = " . WHTELABEL_ID
                    )->result_array();
                    ...
                    if (!empty($whitelabel_childs)) {
                        ...
                        FROM " . ACCOUNT_DATABASE_NAME . "
                        WHERE user_id = " . WHTELABEL_ID . " AND entry_type IN (1,2,5)
                    ")->row();
                        $available_balance = isset($res1->available_balance) ? $res1->available_balance : 0;

                        $res2 = $this->db->query("
                        SELECT SUM(amount) AS down_level_balances
                        FROM " . ACCOUNT_DATABASE_NAME . "
                        WHERE user_id IN ($whtlbl_child_ids)
                    ")->row();
                        $down_level_balances = isset($res2->down_level_balances) ? $res2->down_level_balances : 0;

                        $pl_data = $this->db->query("
                        SELECT SUM(amount) AS my_pl
                        FROM " . ACCOUNT_DATABASE_NAME . "
                        WHERE user_id = " . WHTELABEL_ID . " AND entry_type NOT IN (1,2,5)
                    ")->row();
                        $my_pl = isset($pl_data->my_pl) ? $pl_data->my_pl : 0;

                        $account_balance_whitelabel = ($available_balance + $my_pl) + $down_level_balances;
                    }
                }
            }
            $parent_Status = $parent_bet_status = $parent_cricket_access = $parent_soccer_access = $parent_tennis_access = $parent_video_access = 1;
            ...
            if ($this->input->post('agent_id')) {
                ...
                if (!empty($decrypted_id)) {
                    ...
                    if (isset($ex_ids[1]) && $ex_ids[0] == $user_data['Id']) {
                        $agent_id_id = $ex_ids[1];

                        $get_all_parent_id = $this->db->query("
                            SELECT Id, parent_id, parentDL, parentMDL, parentSuperMDL, parentKingAdmin
                            FROM user_master
                            WHERE Id = $agent_id_id
                        ")->result_array();

                        $all_agent_parent_id = [];
                        foreach ($get_all_parent_id as $parent_id) {
                            foreach (['Id', 'parent_id', 'parentDL', 'parentMDL', 'parentSuperMDL', 'parentKingAdmin'] as $key) {
                                if (!empty($parent_id[$key])) {
                                    $all_agent_parent_id[] = $parent_id[$key];
                                }
                            }
                        }

                        $all_agent_parent_id = array_unique($all_agent_parent_id);
                        if (!empty($all_agent_parent_id)) {
                            $all_agent_parent_id_text = implode(",", $all_agent_parent_id);
                            $get_agent_data = $this->db->query("
                                SELECT cricket_access, soccer_access, tennis_access, video_access, bet_status, Status
                                FROM user_master
                                WHERE Id IN ($all_agent_parent_id_text)
                            ")->result_array();

                            foreach ($get_agent_data as $agent_data) {
                                if (empty($agent_data['cricket_access'])) $parent_cricket_access = 0;
                                if (empty($agent_data['soccer_access'])) $parent_soccer_access = 0;
                                if (empty($agent_data['tennis_access'])) $parent_tennis_access = 0;
                                if (empty($agent_data['video_access'])) $parent_video_access = 0;
                                if (empty($agent_data['Status'])) $parent_Status = 0;
                                if (empty($agent_data['bet_status'])) $parent_bet_status = 0;
                            }
                        }
                    }
                }
            ...
            foreach ($users_list as $key => $ul) {
                ...
                $account_balance = $account_balance1 = $betting_liability = $master_betting_liability = 0;

                if ($user2 == 'show') {
                    $down_ids = isset($parent_children_map[$ul['Id']]) ? array_unique($parent_children_map[$ul['Id']]) : array();
                    $down_ids[] = $ul['Id'];



                    foreach ($down_ids as $did) {
                        $account_balance += isset($balances_map[$did]) ? $balances_map[$did] : 0;

                        $betting_liability += (isset($liabilities_map[$did]) && isset($liabilities_map[$did]['betting_liability']))
                            ? $liabilities_map[$did]['betting_liability']
                            : 0;

                        $master_betting_liability += (isset($liabilities_map[$did]) && isset($liabilities_map[$did]['master_betting_liability']))
                            ? $liabilities_map[$did]['master_betting_liability']
                            : 0;
                    }

                    $account_balance1 = isset($balances_map[$ul['Id']]) ? $balances_map[$ul['Id']] : 0;

                    if ($user_data['power'] == 6 && !$this->input->post('agent_id', TRUE)) {
                        $account_balance = $account_balance_whitelabel;
                    }
                ...
                if (!$this->input->post('agent_id', TRUE)) {
                    ...
                    if ($user2 == 'hide') {
                        $buttons .= '<button type="button" title="" style="margin-right:2px;" data-toggle="tooltip" data-placement="top" class="btn-custom-button btn_more" id="' . $ul['Id'] . '" data-original-title="More">More</button> ';
                    }
                }
                ...
                if ($user2 == 'hide') {
                    ...
                    $agent_url = ($ul['power'] == 1) ? 'javascript:void(0);' : '' . MASTER_URL . '/manage-clients/listing2/' . $user_id;
                }
                ...
                if ($user2 == 'show') {
                    $html .= '<td class="amount-field">' . ROUND($account_balance, 2) . '</td>'
            ...
            if (count($users_list) > 1) {
                $html_header = '<tr>' .
                    '   <td></td>' .
                    '   <td class="amount-field"><b>' . number_format($total_credit, 2) . '</b></td>';
                if ($user2 == 'show') {
                    $html_header .= '<td class="amount-field"><b>' . number_format($total_balance, 2) . '</b></td>' .
                        '   <td class="amount-field"><b>' . number_format($total_client_pl, 2) . '</b></td>' .
                        '   <td class="amount-field"><b>' . number_format($total_exposure, 2) . '</b></td>' .
                        '   <td class="amount-field"><b>' . number_format($total_avl_balance, 2) . '</b></td>';
                }
                $html_header .= '<td></td><td></td><td></td><td></td><td></td><td></td>';
        ...
        $data['result'] = $html;
        $data['down_ids'] = $down_ids;
        $data['balances_map'] = $balances_map;
        $data['users_list'] = $users_list;

        echo json_encode($data);
    }

    private function get_users($limit = '', $offset = '', $user_status = '1')
    {
        ...
        if ($encrypt_id = $this->input->post('agent_id', TRUE)) {
            ...
            if (!empty($decrypted_id)) {
                ...
                $agent_id = $ex_ids[1];
                $user_auth_status_data = $this->users->data($agent_id);
                $aaStatus = $user_auth_status_data['Status'];
                $aabet_status = $user_auth_status_data['bet_status'];
                if ($aaStatus == "0" || $aabet_status == "0") {
                    $agent_status = 0;
                    if ($user_status == "1") {
                        return array();
                    }
                }
                if ($user_auth_status_data['parentDL'] != 0) {
                    $parentDL = $user_auth_status_data['parentDL'];
                    $dl_auth_status_data = $this->users->data($parentDL);
                    $aaStatus = $dl_auth_status_data['Status'];
                    $aabet_status = $dl_auth_status_data['bet_status'];
                    if ($aaStatus == "0" || $aabet_status == "0") {
                        $agent_status = 0;
                        if ($user_status == "1") {
                            return array();
                ...
                if ($user_auth_status_data['parentMDL'] != 0) {
                    $parentMDL = $user_auth_status_data['parentMDL'];
                    $mdl_auth_status_data = $this->users->data($parentMDL);
                    $aaStatus = $mdl_auth_status_data['Status'];
                    $aabet_status = $mdl_auth_status_data['bet_status'];
                    if ($aaStatus == "0" || $aabet_status == "0") {
                        $agent_status = 0;
                        if ($user_status == "1") {
                            return array();
                ...
                if ($user_auth_status_data['parentSuperMDL'] != 0) {
                    $parentSuperMDL = $user_auth_status_data['parentSuperMDL'];
                    $supermdl_auth_status_data = $this->users->data($parentSuperMDL);
                    $aaStatus = $supermdl_auth_status_data['Status'];
                    $aabet_status = $supermdl_auth_status_data['bet_status'];
                    if ($aaStatus == "0" || $aabet_status == "0") {
                        $agent_status = 0;
                        if ($user_status == "1") {
                            return array();
                ...
                if ($user_auth_status_data['parentKingAdmin'] != 0) {
                    $parentKingAdmin = $user_auth_status_data['parentKingAdmin'];
                    $king_auth_status_data = $this->users->data($parentKingAdmin);
                    $aaStatus = $king_auth_status_data['Status'];
                    $aabet_status = $king_auth_status_data['bet_status'];
                    if ($aaStatus == "0" || $aabet_status == "0") {
                        $agent_status = 0;
                        if ($user_status == "1") {
                            return array();
        ...
        if ($user_status == "1") {
            ...
            $this->db->where('u.bet_status', '1');
        } else if ($user_status == "0") {
            if ($agent_status == "1") {
                $this->db->where("(u.Status='0' OR u.bet_status='0')", NULL, FALSE);
            }
        }
    ...
    public function add_user()
    {
        ...
        $min_fancy_stake = $this->input->post('user-minimum_limit_cricket');
        $max_fancy_stake = $this->input->post('user-maximum_limit_cricket');

        $min_cricket_stake = $this->input->post('user-minimum_limit_cricket');
        $max_cricket_stake = $this->input->post('user-maximum_limit_cricket');
        $min_soccer_stake = $this->input->post('user-minimum_limit_cricket');
        $max_soccer_stake = $this->input->post('user-maximum_limit_cricket');
        $min_tennis_stake = $this->input->post('user-minimum_limit_cricket');
        $max_tennis_stake = $this->input->post('user-maximum_limit_cricket');
        $min_casino_stake = $this->input->post('user-minimum_limit_cricket');
        $max_casino_stake = $this->input->post('user-maximum_limit_cricket');
    ...
    private function db_trans($user_id = 0, $to_user_ud = 0, $transaction_type = 0, $transaction_points = 0, $transaction_remark = '', $transaction_time = 0, $login_user_id = 0)
    {
        ...
        $user_parent_dl = $user_data['parentDL'];
        $user_parent_mdl = $user_data['parentMDL'];
        $user_parent_smdl = $user_data['parentSuperMDL'];
        $user_parent_king = $user_data['parentKingAdmin'];

        $customer_data = $this->db->query("select * from user_master where Id=$to_user_ud")->row_array();

        $customer_name = $customer_data['Email_ID'];
        $to_parent_id = $customer_data['parent_id'];
        $to_parent_dl = $customer_data['parentDL'];
        $to_parent_mdl = $customer_data['parentMDL'];
        $to_parent_smdl = $customer_data['parentSuperMDL'];
        $to_parent_king = $customer_data['parentKingAdmin'];
        if ($login_user_id != 0) {
            if ($login_user_id != $user_id && $user_parent_id != $login_user_id && $user_parent_dl != $login_user_id && $user_parent_mdl != $login_user_id && $user_parent_smdl != $login_user_id && $user_parent_king != $login_user_id) {
                $return_array = array(
                    "status" => "error",
                    "message" => "Something Went Wrong, Please try again!"
                );
            ...
            if ($to_parent_id != $login_user_id && $to_parent_dl != $login_user_id && $to_parent_mdl != $login_user_id && $to_parent_smdl != $login_user_id && $to_parent_king != $login_user_id) {
                $return_array = array(
                    "status" => "error",
    ...
    public function change_status()
    {
        ...
        if ($id == LOGINDEMOID) {
            $return_array = array(
                "status" => "error",
                "msg" => "This ID is Login Demo so can not change status."
            );
            return $this->print_json($return_array);
        }
    ...
    public function change_access()
    {
        ...
        echo $salted_hash = hash('sha256', $this->input->post('master_password') . $user_password_salt_key . $user_password_salt);
    ...
    public function changepwd()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($this->db->update('user_login_master', array('Password' => $salted_hash, 'Password2' => $salted_hash, 'user_password_salt_key' => $site_salt, 'user_password_salt' => $p_salt, 'first_password_changed' => 0, 'transaction_password' => $salted_hash, 'transaction_password_salt' => $p_salt, 'transaction_password_salt_key' => $site_salt, 'loginString' => $login_random_string), array('UserID' => $id), 1)) {
                ...
                $this->db->update('user_master', array('user_verification_status' => 'DISABLED', 'user_verification_type' => NULL, 'user_device_id' => NULL), array('Id' => $id));
