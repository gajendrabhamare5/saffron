...
class Bollywood_casino extends CI_Controller {
    ...
    public function index() {
        ...
        $this->layout['title'] = PROJECTNAME . ' - DRAGON TIGER';

        $this->load->view('header', array('handler' => $this));
        $this->load->view('live-market/bollywood-casino/list', array('handler' => $this));
    ...
    public function aaa() {
        ...
        $this->layout['title'] = PROJECTNAME . ' - AMAR AKBAR ANTHONY';

        $this->load->view('header', array('handler' => $this));
        $this->load->view('live-market/bollywood-casino/aaa', array('handler' => $this));
        $this->load->view('footer', array('handler' => $this));
    }
    ...
    public function btable() {
        ...
        $this->layout['title'] = PROJECTNAME . ' - BOLLYWOOD TABLE';

        $this->load->view('header', array('handler' => $this));
        $this->load->view('live-market/bollywood-casino/btable', array('handler' => $this));
        $this->load->view('footer', array('handler' => $this));
    }

     public function btable2() {
        ...
        $this->layout['title'] = PROJECTNAME . ' - BOLLYWOOD TABLE 2';

        $this->load->view('header', array('handler' => $this));
        $this->load->view('live-market/bollywood-casino/btable2', array('handler' => $this));
        $this->load->view('footer', array('handler' => $this));
    }
     ...
     public function aaa2() {
        ...
        $this->layout['title'] = PROJECTNAME . ' - Amar Akbar Anthony 2';

        $this->load->view('header', array('handler' => $this));
        $this->load->view('live-market/bollywood-casino/aaa2', array('handler' => $this));
        $this->load->view('footer', array('handler' => $this));
    }

    public function active_bets($type = 'btable', $event_id = '') {
        ...
        $shortcodeArr = array('btable' => 'B_TABLE', 'aaa' => 'AMAR_AKBAR_ANTHONY','btable2' => 'B_TABLE2','aaa2' => 'AMAR_AKBAR_ANTHONY2');
    ...
    public function view_more_matched($game = 'btable', $event_id = '') {
        $this->load->database();
        $this->load->library('users', $this);
    ...
    public function market_analysis($type = 'btable', $event_id = '') {
...
//            $this->db->where(array('b.event_id' => $event_id, 'b.event_type' => $shortcodeArr[$type]));
            ...
            if ($bets_data) {
                ...
                if ($type == 'btable'|| $type == 'btable2'|| $type == 'aaa') {

                    foreach ($all_markets as $all_market) {
                        if (!in_array($all_market->market_id, array(1, 2, 3, 4, 5, 6))) {
                            continue;
                        }

                        $pl = 0;
                        foreach ($bets_data as $bet_data) {
                            if (!in_array($bet_data['market_id'], array(1, 2, 3, 4, 5, 6))) {
                                continue;
                            }
                    ...
                    foreach ($all_markets as $all_market) {
                        if (!in_array($all_market->market_id, array(7))) {
                            continue;
                        }

                        $pl = 0;
                        foreach ($bets_data as $bet_data) {
                            if (!in_array($bet_data['market_id'], array(7))) {
                                continue;
                            }
                    ...
                    foreach ($bets_data as $bet_data) {
                        ...
                        if (in_array($bet_data['market_id'], array(1, 2, 3, 4, 5, 6, 7))) {
                            continue;
                        }
                    ...
                    foreach ($all_markets as $all_market) {
                        if (!in_array($all_market->market_id, array(1, 2, 3))) {
                            continue;
                        }

                        $pl = 0;
                        foreach ($bets_data as $bet_data) {
                            if (!in_array($bet_data['market_id'], array(1, 2, 3))) {
                                continue;
                            }
