...
<div class="col-md-12 main-container">
  ...
  <label>
    ...
    entries
                                ...
                                <div id="tickets-table_filter" class="dataTables_filter text-md-right">
                                    <label class="d-inline-flex align-items-center"> Search:
                                        ...
                                        <!-- <button type="button" id="btn-replica-hisab" class="btn btn-warning ml-2" title="Table vs header hisab diagnostic">Hisab Diagnostic</button> -->
                                    </label>
                                </div>
    ...
    <div class="modal fade" id="replica-hisab-modal">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Hisab Diagnostic</h4>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <div class="modal-body">
                    <div id="replica-hisab-summary" class="alert alert-info small mb-3"></div>
                    <div class="table-responsive">
                        <table class="table table-sm table-bordered" id="replica-hisab-table">
                            <thead>
                                <tr>
                                    <th>User</th>
                                    <th>Credit Ref</th>
                            ...
                            <tbody id="replica-hisab-tbody"></tbody>
                        </table>
                    </div>
                </div>
    ...
    $('#btn-reset').on('click', function () {
        ...
        $('#manage_user_tbody').delegate('.btn_deposit_modal', 'click', function() {
            ...
            $('#deposit_master_password').val('');
             $('#deposit_amount').val('');
            $('#deposite-first-diff').text('');
            $('#deposite-second-diff').text('');
        ...
        $('#manage_user_tbody').delegate('.btn_withdrawal_modal', 'click', function() {
            ...
            $('#withdraw-first-diff').text('');
            $('#withdraw-second-diff').text('');
        ...
        $('#btn-replica-hisab').on('click', function() {
            $('#divLoading').addClass('show');
            var postdata = preparesearch() + prepareorderby();
            $.ajax({
                url: MASTER_URL + '/manage_clients/get_user_list_replica/0/' + user_status + '/show',
                type: 'POST',
                dataType: 'json',
                data: postdata,
                success: function(data) {
                    ...
                    if (!data || !data.compare) {
                        alert('No compare data returned.');
                        return;
                    }
                    var diag = data.diagnostics || {};
                    var hdr = data.header_reference || {};
                    var matchClass = diag.header_matches_table ? 'alert-success' : 'alert-warning';
                    var summaryHtml = '<div class="' + matchClass + ' p-2 mb-2"><strong>Table footer P/L:</strong> ' + diag.table_footer_pl +
                        ' &nbsp;|&nbsp; <strong>Header Down Level P/L (fixed):</strong> ' + diag.header_down_pl_live +
                        (diag.header_matches_table ? ' ✓ aligned' : ' ⚠ mismatch — refresh account summary in header') +
                        '<br><strong>Header P/L (old wrong formula):</strong> ' + diag.header_down_pl_old_formula +
                        ' <small class="text-muted">(was credit−balance, opposite sign)</small></div>';

                    if (data.summary.unchanged_rows === data.summary.rows && diag.balance_pl_identical_reason) {
                        summaryHtml += '<div class="alert alert-secondary small mb-2"><strong>Balance &amp; P/L old=new is normal</strong> for your data:<ul class="mb-0">';
                        diag.balance_pl_identical_reason.forEach(function(r) {
                            summaryHtml += '<li>' + r + '</li>';
                        });
                        summaryHtml += '</ul>Main fix visible here: <strong>header Down Level P/L</strong> now matches table footer (same sign &amp; formula).</div>';
                    } else {
                        summaryHtml += '<div class="small mb-2">Rows changed: ' + data.summary.changed_rows + ' / ' + data.summary.rows + '</div>';
                    }
                    $('#replica-hisab-summary').html(summaryHtml);

                    var rowsHtml = '';
                    data.compare.forEach(function(row) {
                        var avlChanged = Math.abs(row.delta.avl_balance) > 0.009;
                        var rowClass = (row.changed && avlChanged) ? 'table-warning' : '';
                        rowsHtml += '<tr class="' + rowClass + '">' +
                            '<td>' + row.email + '</td>' +
                            '<td class="text-right">' + row.credit_reference + '</td>' +
                            '<td class="text-right">' + row.replica.balance + '</td>' +
                            '<td class="text-right">' + row.replica.client_pl + '</td>' +
                            '<td class="text-right">' + row.legacy.avl_balance + '</td>' +
                            '<td class="text-right">' + row.replica.avl_balance + '</td>' +
                            '<td class="text-right"><b>' + row.delta.avl_balance + '</b></td>' +
                            '<td class="text-right">' + row.replica.downline_users + '</td>' +
                            '</tr>';
                    });
                    $('#replica-hisab-tbody').html(rowsHtml);
                    $('#replica-hisab-modal').modal('show');
                },
                error: function() {
                    $('#divLoading').removeClass('show');
                    alert('Failed to load replica hisab compare.');
                }
            });
        });
    });
