            ...
            <div class="col-md-8 main-content">
                <div class="coupon-card">
                    <div class="game-heading"><span class="card-header-title">Teenpatti Poison 20-20
                        ...
                        <!-- <small role="button" class="teenpatti-rules" onclick="view_rules('poison20')" data-target="#rules_popup" data-toggle="modal" tabindex="0"><u>Rules</u></small>  -->
                    ...
                    <div class="video-container">
                        ...
                        ?>
                            <iframe class="iframedesign" src="<?php echo IFRAME_URL . "" . POISON20_CODE; ?>" width="100%" height="400" style="border: 0px;"></iframe>
                        <?php
                                        ...
                                        <div class="rules-body">
                                            <div>
                                                ...
                                                <div class="rules-section">
                                                    <p>Welcome to Teenpatti Poison 20-20, a new variation of Teenpatti.</p>
                                                    <p>As Teenpatti games are becoming more and more famous and popular on our platforms, we are excited to introduce you to Teenpatti Poison 20-20. The game follows the same standard rules of Teenpatti but at the beginning of the round the dealer draws a Poison card before dealing to the players. The Poison card is toxic and makes the player lose as soon as any player gets it. If no Poison card is dealt then the game continues as per Teenpatti standard rules.
                                                    </p>
                                                    <p>For Example:</p>
                    ...
                    <div class="casino-detail">
                        <div class="teen1daycasino-container mt-2">
                            ...
                            <!----> 
                            <div class="casino-last-result-title"><span>Last Result</span><span><a href="<?php echo MASTER_URL; ?>/reports/casino-results?q=poison20">View All</a></span></div>
...
<script>
    function view_casiono_result(event_id, casino_type) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo MASTER_URL; ?>/reports/Casino_results/get_result/poison20/' + event_id,
            dataType: 'json',
            data: {
                event_id: event_id,
    ...
    function call_active_bets() {
        if (get_round_no() > 0) {
            if (xhr && xhr.readyState != 4)
                xhr.abort();
            xhr = $.ajax({
                url: MASTER_URL + '/live-market/teenpatti/active_bets/poison20/' + get_round_no(),
                type: 'POST',
                dataType: 'json',
    ...
    function call_view_more_bets() {
        if (xhr_vm && xhr_vm.readyState != 4)
            xhr_vm.abort();
        xhr_vm = $.ajax({
            url: MASTER_URL + '/live-market/teenpatti/view_more_matched/poison20/' + get_round_no(),
            type: 'POST',
            dataType: 'json',
    ...
    function call_analysis() {
        if (xhr_analysis && xhr_analysis.readyState != 4)
            xhr_analysis.abort();
        xhr_analysis = $.ajax({
            url: MASTER_URL + '/live-market/teenpatti/market_analysis/poison20/' + get_round_no(),
            type: 'POST',
            dataType: 'json',
            success: function(data) {
                if (data.results == 0) {
