		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-header">
					<h4 class="modal-title">Market Analysis</h4>
				...
				<div class="table-responsive data-table" style="overflow: hidden;margin-top:10px"
					id="analysis_html1_footer">
...
<script type="text/javascript">
	var websocketurl = '<?php echo SITE_SPORTS_IP; ?>';
	var socket123 = io("<?php echo SITE_SPORTS_IP; ?>");
...
<script type="text/javascript">
	...
	var xhr_footer;
	function call_xhr_footer(url, mydata) {
		if (xhr_footer && xhr_footer.readyState != 4)
			xhr_footer.abort();
		xhr_footer = $.ajax({
			url: url,
			type: 'POST',
	...
	function call_page_footer(userId) {

		call_xhr_footer(MASTER_URL + '/market_analysis/get_analysis/', { uid: userId });
		xhr_footer.success(function (data) {

			var analysis_html1_footer = '';
			...
			if (typeof (data.results) != "undefined" && data.results.length > 0) {
				...
				$.each(data.results, function (index1, event_data) {
					...
					var market_column_footer = event_data['market_pl'].length;
					var event_id_footer = event_data.event_id;

					var marketTypeMap = {};
					if (data.ytfy && data.ytfy[event_id_footer]) {
						$.each(data.ytfy[event_id_footer], function (i, mkt) {
							marketTypeMap[mkt.market_id] = mkt.market_type || 'Unknown';
						});
					}

					analysis_html1_footer += '<div class="card mb-3" style="padding: 1rem;">';
					analysis_html1_footer +=
					...
					// Card Body Start
					analysis_html1_footer += '<div class="card-body p-2">';
					...
					analysis_html1_footer += '<div class="row">';

					$.each(marketTypeGroups, function (market_type, markets) {
						analysis_html1_footer += '<div class="col-sm-12 col-xl-4 mb-3">';

						// Market Type Title
						analysis_html1_footer +=
							'<h6 class="mb-1 text-uppercase border-bottom pb-1" style="background-color: rgba(0, 0, 0, 0.05);border: 1px solid #dee2e6;text-align: left;padding-left: 5px;padding-top: 5px;">' +
							market_type + '</h6>';
						...
						analysis_html1_footer += '</div>';
					});

					analysis_html1_footer += '</div>';

					analysis_html1_footer += '</div></div>';
				});

				$('#analysis_html1_footer').html(analysis_html1_footer);
			}
		});
	}
