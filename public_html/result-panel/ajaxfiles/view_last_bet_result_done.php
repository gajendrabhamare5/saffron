<?php
	...
	$login_type = $_SESSION['CONTROLLER_LOGIN_SESSION_TYPE'];
	 ...
	 if($login_type == 5){
		...
		$get_pnl_report  = $conn->query("select * from bet_details where 1=1  $search_event_name and bet_time>='$current_date' and bet_status In (1,0) and market_type='FANCY_ODDS' GROUP BY event_id,market_id ORDER BY bet_result_time $limit_query");
	...
	while($fetch_get_pnl_report = mysqli_fetch_assoc($get_pnl_report)){
		...
		$event_id = $fetch_get_pnl_report['event_id'];
		$market_id = $fetch_get_pnl_report['market_id'];
	...
	function date_compare($element1, $element2) {
		$datetime1 = strtotime($element1['result_date_time']);
		$datetime2 = strtotime($element2['result_date_time']);
