...
if(CASINO_MAINTENANCE == 1 and $user_id !=4 and $user_id !=284 and $user_id !=408 and $user_id !=810 ){
	 $return = array(
        "status" => 'error',
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='card32eu'");
...
$url = CASINO_DATA_URL ."?type=race20";
$ch = curl_init();
...
$api_event_id = str_replace("46.","",$api_event_id);
...
if($type == "No" or $type == "Lay"){
				...
				if($marketId == 5 or $marketId == 6){
					
					if($marketId == 5){
						$margin_used = $stack * (2 - 1);
						
					}
					else{
						
						$margin_used = $stack * (2.05 - 1);
					}
				}
				else{
				...
				if($marketId == 5 or $marketId == 6){
					...
					if($marketId == 5){
						...
						$bet_win_amount = $stack * (2 - 1);
					}
					else{
						$bet_win_amount = $stack * (1.90 - 1);
...
$bet_market_name = "";
if($marketId >= 1 and $marketId <= 4){
	$bet_market_name = $market_type;
}
/* else if($marketId == 5){
	$bet_market_name = "TOTAL_POINT";
}else if($marketId == 6){
	$bet_market_name = "TOTAL_CARD";
} */
else{
	$bet_market_name = "";
}
