...
while ($fetch_get_all_bet = mysqli_fetch_assoc($get_all_bet)) {
   ...
   /*  if ((int)$bet_market_id == 7) {
    ...
    $result_list_odds=array();
    if ((int)$bet_market_id == 3) {
       
        if (strpos($result_list[1], "A : ") !== false) {
            $result_list_odds=explode("A : ",$result_list[1]);
            $winner_flag=true;
        }
		
	}else if ((int)$bet_market_id == 4) {
       
        if (strpos($result_list[1], "B : ") !== false) {
            $result_list_odds=explode("B : ",$result_list[1]);
            $winner_flag=true;
        }
    ...
    if ($winner_flag) {
        ...
        if(count($result_list_odds) > 0){
            if($result_list_odds[1] == "Card 9"){
                    $actual_win_amount2 = ($bet_stack * ($bet_odds)) * 3;
            }
            else if($result_list_odds[1] == "Card 8"){
                    $actual_win_amount2 = ($bet_stack * ($bet_odds)) * 4;
            }
            else if($result_list_odds[1] == "Card 7"){
                    $actual_win_amount2 = ($bet_stack * ($bet_odds)) * 5;
            }
            else if($result_list_odds[1] == "Card 6"){
                    $actual_win_amount2 = ($bet_stack * ($bet_odds)) * 8;
            }
            else if($result_list_odds[1] == "Card 5"){
                    $actual_win_amount2 = ($bet_stack * ($bet_odds)) * 30;
            }
        }
