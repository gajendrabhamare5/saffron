...
$event_type = "AMAR_AKBAR_ANTHONY2";
...
$is_even = $rdesc_array[1];
$is_red = $rdesc_array[2];
$is_over_under = $rdesc_array[3];
$is_current_card = $rdesc_array[4];
...
if($is_over_under == "Under7"){
	 $market_id[] = 21;
	 $bet_final_result .= ' || Under 7';
}
else if($is_over_under == "Over7"){
	 $market_id[] = 22;
	 $bet_final_result .= ' || Over 7';
}


$bet_final_result .= '|| Card '.$is_current_card;

$cards_key = is_current_card($is_current_card);
$market_id[] = $cards_key;
...
while ($fetch_get_all_bet = mysqli_fetch_assoc($get_all_bet)) {
    ...
    if ($bet_market_id >= 4) {
        ...
        if (in_array($bet_market_id, $market_id)) {
            ...
            $update_bet = $conn->query("update bet_teen_details set bet_status=0,bet_result='$actual_win_amount2',bet_final_result='$bet_final_result' where bet_id='$bet_id' and bet_status=1");
			...
			if($conn->affected_rows > 0){
                ...
                if(INSERT_ACCOUNTS_TEMP && (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $actual_win_amount2 != 0))){
					$conn->query("insert into accounts_temp (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`event_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`game_type`,`transaction_id`) VALUES('$bet_user_id','$parentSuperMDL','$account_description','$bet_id','$mid','$actual_win_amount2','Credit','4','$transaction_time','1',1,'$transaction_id')");
				}
			...
			if($conn->affected_rows > 0){
                ...
                if(INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $bet_amount != 0)){
				    $insert_user_account = $conn->query("insert into accounts (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`event_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`game_type`,`transaction_id`) VALUES('$bet_user_id','$parentSuperMDL','$account_description','$bet_id','$mid','-$bet_amount','Debit','7','$transaction_time','1',1,'$transaction_id')");
                }
                if(INSERT_ACCOUNTS_TEMP && (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $bet_amount != 0))){
					$conn->query("insert into accounts_temp (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`event_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`game_type`,`transaction_id`) VALUES('$bet_user_id','$parentSuperMDL','$account_description','$bet_id','$mid','-$bet_amount','Debit','7','$transaction_time','1',1,'$transaction_id')");
				}
