...
if ($result == 1 || $result == 'A') {
    $result_status = 'A';
    $bet_final_result = 'DON';
    $market_id_fixed = 1;
} else if ($result == 2 || $result == 'B') {
    $result_status = 'B';
    $bet_final_result = 'Amar Akbar Anthony';
    $market_id_fixed = 2;
} else if ($result == 3 || $result == 'C') {
    $result_status = 'C';
    $bet_final_result = 'Sahib Bibi Aur Ghulam';
    $market_id_fixed = 3;
} else if ($result == 4 || $result == 'D') {
    $result_status = 'D';
    $bet_final_result = 'Dharam Veer';
    $market_id_fixed = 4;
} else if ($result == 5 || $result == 'E') {
    $result_status = 'E';
    $bet_final_result = 'Kis KisKo Pyaar Karoon';
    $market_id_fixed = 5;
} else if ($result == 6 || $result == 'F') {
    $result_status = 'F';
    $bet_final_result = 'Ghulam';
    $market_id_fixed = 6;
} else if ($result == "CAN") {
...
$is_barati = $rdesc_array[2];
$is_red = $rdesc_array[3];
...
if($is_barati == "Barati"){
	 $market_id[] = 15;
    $bet_final_result .= ' | Barati';
}
else{
	$market_id[] = 14;
    $bet_final_result .= ' | Dulha Dulhan';
}
...
while ($fetch_get_all_bet = mysqli_fetch_assoc($get_all_bet)) {
    ...
    if ($bet_market_id <= 6) {
        if ($bet_market_id == $market_id_fixed and $bet_type == "Back") {
            set_result($conn, $bet_id, "Win", $mid, $end_date_time, $bet_final_result,$event_type);
        } else if ($bet_market_id != $market_id_fixed and $bet_type == "Lay") {
            set_result($conn, $bet_id, "Win", $mid, $end_date_time, $bet_final_result,$event_type);
        } else {
            set_result($conn, $bet_id, "Loss", $mid, $end_date_time, $bet_final_result,$event_type);
        }
    }

    if ($bet_market_id == 7) {

//        if ($bet_market_id == $market_id_odd and $bet_type == "Back") {
//            set_result($conn, $bet_id, "Win", $mid, $end_date_time);
//        } else if ($bet_market_id != $market_id_odd and $bet_type == "Lay") {
//            set_result($conn, $bet_id, "Win", $mid, $end_date_time);
//        } else {
...
//        }
        ...
        if($bet_type == "Back"){
            ...
            if($bet_market_id == $market_id_odd){
                set_result($conn, $bet_id, "Win", $mid, $end_date_time, $bet_final_result,$event_type);
            }
            else{
                set_result($conn, $bet_id, "Loss", $mid, $end_date_time, $bet_final_result,$event_type);
...
function set_result($conn, $bet_id, $result_status, $mid, $end_date_time, $bet_final_result='',$event_type='') {
    ...
    if ($result_status == 'Win') {
		...
		if($conn->affected_rows > 0){
			...
			foreach ($level_pers as $cradit_user_id => $level_per) {
				...
				$cradit_amt = ($user_amount / 100) * $level_per;
				$transaction_id = $bet_id.'-'.$cradit_user_id;
