...
$point = str_replace(":","",$point);
$total_point = str_replace("Point","",$point);

$cards = $rdesc_array[2];
$cards = str_replace(" ","",$cards);
$cards = str_replace(":","",$cards);
$total_cards = str_replace("Cards","",$cards);

if($total_cards == 15){
	$market_id[] = 10;
}
else if($total_cards == 16){
	$market_id[] = 11;
}
else if($total_cards == 17){
	$market_id[] = 12;
}
...
while ($fetch_get_all_bet = mysqli_fetch_assoc($get_all_bet)) {
	...
	if ($bet_market_id == 5) {
		...
		if($bet_type == "Back" and $bet_odds<= $total_point){
			set_result($conn,$bet_id,'Win', $mid, $end_date_time, $bet_final_result,$event_type);
		}else if($bet_type=="Lay" and $bet_odds > $total_point){
			set_result($conn,$bet_id,'Win', $mid, $end_date_time, $bet_final_result,$event_type);
		}else{
			set_result($conn,$bet_id,'Loss', $mid, $end_date_time, $bet_final_result,$event_type);
		}
	...
	if ($bet_market_id == 6) {
        if($bet_type == "Back" and $bet_odds<= $total_cards){
			set_result($conn,$bet_id,'Win', $mid, $end_date_time, $bet_final_result,$event_type);
		}else if($bet_type=="Lay" and $bet_odds > $total_cards){
			set_result($conn,$bet_id,'Win', $mid, $end_date_time, $bet_final_result,$event_type);
		}else{
			set_result($conn,$bet_id,'Loss', $mid, $end_date_time, $bet_final_result,$event_type);
		}
