...
class Events_analysis3 extends CI_Controller
{
	...
	public function index($event_id = '')
	{
		...
		$this->load->view('events_analysis/events_analysis3', array('handler' => $this));
	...
	public function get_analysis($event_id = '')
	{
		...
		if ($event_id == "") {
			$sql_event_id = "SELECT b.event_id FROM bet_details b  WHERE b.bet_status in (0,1) and b.user_id IN ($downline_user_list) GROUP BY b.event_id";
		...
		if ($bets_data) {
			...
			$market_pl_matchtied = $this->get_market_exposure($markets_data, $bets_data, 'TIED_MATCH', $user_data, $self_percentage_array);

			if ($event_id != "") {
				$market_pl_book = $this->get_market_exposure($markets_data, $bets_data, 'BOOKMAKER_ODDS', $user_data, $self_percentage_array);
				$market_pl_booktied = $this->get_market_exposure($markets_data, $bets_data, 'BOOKMAKER_TIED_ODDS', $user_data, $self_percentage_array);
		...
		if (!empty($market_pl_matchtied)) {
			$market_pl = array_merge($market_pl, $market_pl_matchtied);
		}

		if (!empty($market_pl_book)) {
			$market_pl = array_merge($market_pl, $market_pl_book);
		}
		if (!empty($market_pl_booktied)) {
			$market_pl = array_merge($market_pl, $market_pl_booktied);
		}
	...
	public function get_analysis__old($event_id = '')
	{
		...
		$sql_event_id = "SELECT b.event_id FROM bet_details b LEFT JOIN user_master u ON u.Id = b.user_id WHERE b.bet_status in (0,1) $where_event GROUP BY b.event_id";
		$event_ids_data = $this->db->query($sql_event_id)->result_array();
		...
		if ($bets_data) {
			...
			$market_pl_matchties = $this->get_market_exposure($markets_data, $bets_data, 'TIED_MATCH', $user_data);
			$market_pl_book = $this->get_market_exposure($markets_data, $bets_data, 'BOOKMAKER_ODDS', $user_data);
			$market_pl_booktied = $this->get_market_exposure($markets_data, $bets_data, 'BOOKMAKER_TIED_ODDS', $user_data);
			$market_pl_booksm1 = $this->get_market_exposure($markets_data, $bets_data, 'BOOKMAKERSMALL_ODDS', $user_data);

			$market_pl1 = array_merge($market_pl_match, $market_pl_book, $market_pl_booksm1, $market_pl_matchties, $market_pl_booktied);

			foreach ($bets_data as $bet_data) {

				if (in_array($bet_data['market_type'], array('MATCH_ODDS', 'BOOKMAKER_ODDS', 'BOOKMAKERSMALL_ODDS', 'TIED_MATCH', 'BOOKMAKER_TIED_ODDS'))) {
					continue;
				}
	...
	public function get_userbook($event_id = '', $market_type = 'MATCH_ODDS', $tree_userid = 0)
	{
		...
		$sql = '
                SELECT e.*
                FROM  event_market_id e 
                WHERE e.event_id IN (SELECT b.event_id FROM bet_details b LEFT JOIN user_master u ON u.Id = b.user_id WHERE b.bet_status in (0,1) AND event_id = ' . $event_id . ' AND market_type = "' . $market_type . '" ' . $where_event . ')
                    AND e.event_id = ' . $event_id . ' 
	...
	public function view_more_matched($event_id = '')
	{
		...
		foreach ($bets_data as $bet_data) {
			...
			$html .= '<td class="text-right"><div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input bet-checkbox" id="pro_los_' . $sno . '" data-wl="' . $bet_wl . '" ><label class="custom-control-label" for="pro_los_' . $sno . '"></label></div></td>';
		...
		return $this->print_json(array('results' => $html, 'total' => $total_bets, 'amttot' => $totalpl));
	}
