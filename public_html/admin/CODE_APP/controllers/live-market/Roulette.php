...
class Roulette extends CI_Controller {
    ...
    public function index() {
        ...
        $this->layout['title'] = PROJECTNAME . ' - Roulette';

        $this->load->view('header', array('handler' => $this));
        $this->load->view('live-market/roulette/list', array('handler' => $this));
        $this->load->view('footer', array('handler' => $this));
    }

    public function ourroullete() {
        ...
        $this->layout['title'] = PROJECTNAME . ' - Unique Roulette';

        $this->load->view('header', array('handler' => $this));
        $this->load->view('live-market/roulette/ourroullete', array('handler' => $this));
        $this->load->view('footer', array('handler' => $this));
    }

    public function roulette11() {
        ...
        $this->layout['title'] = PROJECTNAME . ' - Golden Roulette';

        $this->load->view('header', array('handler' => $this));
        $this->load->view('live-market/roulette/roulette11', array('handler' => $this));
        $this->load->view('footer', array('handler' => $this));
    }

    public function roulette12() {
        ...
        $this->layout['title'] = PROJECTNAME . ' - Beach Roulette';

        $this->load->view('header', array('handler' => $this));
        $this->load->view('live-market/roulette/roulette12', array('handler' => $this));
        $this->load->view('footer', array('handler' => $this));
    }

    public function roulette13() {
        ...
        $this->load->view('header', array('handler' => $this));
        $this->load->view('live-market/roulette/roulette13', array('handler' => $this));
        $this->load->view('footer', array('handler' => $this));
    }

    public function active_bets($type = 'ourroullete', $event_id = '') {
        ...
        $shortcodeArr = array('ourroullete' => 'OURROULLETE', 'roulette11' => 'ROULETTE11', 'roulette12' => 'ROULETTE12', 'roulette13' => 'ROULETTE13');
        $html = '';
    ...
    public function view_more_matched($game = 'ourroullete', $event_id = '') {
        $this->load->database();
        $this->load->library('users', $this);
    ...
    public function market_analysis($type = 'ourroullete', $event_id = '') {
            ...
            if ($bets_data) {
                ...
                if ($type == 'oneday') {
                    ...
                    foreach ($all_markets as $all_market) {
                        if (!in_array($all_market->market_id, array(1, 2))) {
                            continue;
                        }

                        $pl = 0;
                        foreach ($bets_data as $bet_data) {
                            if (!in_array($bet_data['market_id'], array(1, 2))) {
                                continue;
                            }
...
//                    $my_pl = $pl;
                ...
                } else if ($type == 'player6' || $type == 'OURROULLETE') {
                    foreach ($bets_data as $bet_data) {
