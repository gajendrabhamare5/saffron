...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("TEENUNIQUE", $casino_lis_db)){
		$return = array(
		"status" => 'error',
		"message" => CASINO_MAINTENANCE_MSG,
	);
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and lower(casino_name)='teenunique'");
...
if($market_runner_name == ""){
	$return = array(
        "status" => 'error',
        "message" => 'Please select any 3 cards',
    );
	save_rejection_log($conn,$user_id,($return['message']." #".__line__),$market_runner_name,$page_call_time);
    echo json_encode($return);
    exit();
...
$market_runner_name_arr = explode(',',$market_runner_name);

if(count($market_runner_name_arr) != 3){
	$return = array(
        "status"=>'error',
		"message"=>"3 cards are required.",
    );
	save_rejection_log($conn,$user_id,($return['message']." #".__line__),$market_runner_name,$page_call_time);
    echo json_encode($return);
    exit();
...
$url = CASINO_DATA_URL ."?type=teenunique";
$ch = curl_init();
