...
if ($market_type != "FANCY_ODDS") {
	...
	if ($check_num_rows <= 0) {
		...
		$get_1_win_back_data = $conn->query("select * from  bet_details  where user_id=$user_id and bet_status =1 and event_id=$event_id and market_id=$marketId and market_type='FANCY_ODDS'");
		$fetch_get_event_market = mysqli_fetch_assoc($get_1_win_back_data);
		$market_name_db = $fetch_get_event_market['market_name'];
		if (strpos($market_name_db, '(EVEN)') !== false || strpos($market_name_db, '(ODD)') !== false) {
			$exposure_data = get_current_bet_fancy_exposure_oddseven($conn, $user_id, $event_id, $marketId, $current_bet);
		} else {
