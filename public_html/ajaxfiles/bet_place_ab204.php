...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("AB204", $casino_lis_db)){
		$return = array(
		"status" => 'error',
...
$market_odd_name = $conn->real_escape_string($_REQUEST['market_odd_name']);
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='ab4'");
...
$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='ab4';");
...
foreach($t2_datas as $t2_data){
	...
	if($api_sid == $marketId){
		...
		/* if($bet_status != "1" and $bet_status != 1 and $bet_status != 'ACTIVE'){
			$return = array(
				"status"=>'error',
				"message"=>'Bet Suspended',
			);
...
/*if($get_data->odds->t2[0]->gstatus != 1 or $get_data->odds->t2[0]->gstatus != "1"){
	$return = array(
		"status"=>'error',
		"message"=>'Bet Suspended',
	);
...
$api_event_id = str_replace("14.","",$api_event_id);
...
$bet_market_name = "";
/* if($marketId == 1 or $marketId == 21){
	$bet_market_name = "CARDA";
}else if($marketId == 2 or $marketId == 22) {
	$bet_market_name = "CARD2";
 }else if($marketId == 3 or $marketId == 23) {
	$bet_market_name = "CARD3";
 }else if($marketId == 4 or $marketId == 24) {
	$bet_market_name = "CARD4";
 }else if($marketId == 5 or $marketId == 25) {
	$bet_market_name = "CARD5";
 }else if($marketId == 6 or $marketId == 26) {
	$bet_market_name = "CARD6";
 }else if($marketId == 7 or $marketId == 27) {
	$bet_market_name = "CARD7";
 }else if($marketId == 8 or $marketId == 28) {
	$bet_market_name = "CARD8";
 }else if($marketId == 9 or $marketId == 29) {
	$bet_market_name = "CARD9";
 }else if($marketId == 10 or $marketId == 30) {
	$bet_market_name = "CARD10";
 }else if($marketId == 11 or $marketId == 31) {
	$bet_market_name = "CARDJ";
 }else if($marketId == 12 or $marketId == 32) {
	$bet_market_name = "CARDQ";
 }else if($marketId == 13 or $marketId == 33) {
	$bet_market_name = "CARDK";
 } */
