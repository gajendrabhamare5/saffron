...
include('../include/flip_function2_test.php');
...
$serno = 0;
/* $data_pass=json_decode(file_get_contents('php://input')); */
$idssss = '2508463,2440765';
/* $idssss = '1684390'; */
$ggg = $conn->query("SELECT *  FROM `bet_success_log_test` WHERE `bet_id` in ($idssss) ORDER BY bet_id ASC");
while ($daataa = mysqli_fetch_assoc($ggg)) {
    $data_pass = $daataa['log_details'];
    $data_pass = json_decode($data_pass);
    $api_data = $daataa['api_data'];
   
    
    if ($api_data !== '' && $api_data[0] === '"' && substr($api_data, -1) === '"') {
        $api_data = substr($api_data, 1, -1);
    }
    $api_data1 = json_decode($api_data,true);
    if (isset($data_pass)) {
        $_POST = (array)$data_pass;
    }


    $user_id = "11";
    $user_login_name = "democ2";
    ...
    if ($user_id == 12352 && $print_value) {
        echo "usermaster=" . date("Y-m-d H:i:s");
        echo "<br>";
    }
    ...
    if ($market_type == "MATCH_ODDS" || $market_type == "BOOKMAKERSMALL_ODDS" || $market_type == "BOOKMAKER_ODDS" || $market_type == "BOOKMAKER_TIED_ODDS") {
        if ($user_id == 12352 && $print_value) {
            echo "bet_block_details=" . date("Y-m-d H:i:s");
            echo "<br>";
        }
    ...
    if ($user_id == 12352 && $print_value) {
        echo "bet_delay_master=" . date("Y-m-d H:i:s");
        echo "<br>";
    }
    ...
    if ($user_id == 12352 && $print_value) {
        echo "getFancyData1=" . date("Y-m-d H:i:s");
        echo "<br>";
    }
    $data_new = $api_data;
    ...
    if ($user_id == 12352 && $print_value) {
        echo "block_sport=" . date("Y-m-d H:i:s");
        echo "<br>";
    }
    ...
    if ($user_id == 12352 && $print_value) {
        echo "block_event_id=" . date("Y-m-d H:i:s");
        echo "<br>";
    }
    ...
    if ($user_id == 12352 && $print_value) {
        echo "total_balance=" . date("Y-m-d H:i:s");
        echo "<br>";
    }
	...
	if ($type == "No" || $type == "Lay") {
        ...
        if ($user_id == 12352 && $print_value) {
            echo "event_market_limit=" . date("Y-m-d H:i:s");
            echo "<br>";
        }
        if ($eventType != 4) {
        ...
        if ($user_id == 12352 && $print_value) {
            echo "save_rejection_log=" . date("Y-m-d H:i:s");
            echo "<br>";
        }
        $exposure_balance = get_net_exposure_during_placing_v3($conn, $user_id, $current_bet);
        echo "EXPOSURE BALANCE == ".$exposure_balance;
        print_r($exposure_balance);
        $exposure_balance = $exposure_balance['total'];
        echo "</br>";
        if ($user_id == 12352 && $print_value) {
            echo "get_net_exposure_during_placing_v3=" . date("Y-m-d H:i:s");
            echo "<br>";
        }
        ...
        if ($user_id == 12352 && $print_value) {
            echo "SPORTS_DATA_URL=" . date("Y-m-d H:i:s");
            echo "<br>";
        }
        ...
        foreach ($data as $live_market_rate) {
            if ($live_market_rate->SelectionId == $marketId) {
                ...
                $exposure_balance += user_prv_exposure_except_current_cmp($conn, $user_id, $cmp_id, $eventId);
           		echo "EXPOSURE BALANCE RPD == ".$exposure_balance;
           		echo "</br>";
            }
        }
        if ($user_id == 12352 && $print_value) {
        ...
        $exposure_balance = get_net_exposure_during_placing_v3($conn, $user_id, $current_bet);
        echo "</br>";
		echo "PLACING EXPOSURE == ".$exposure_balance;
		print_r($exposure_balance);
		$exposure_balance = $exposure_balance['total'];
		echo "</br>";
    }
    ...
    if ($user_id == 12352 && $print_value) {
        echo "get_unmatched_expo=" . date("Y-m-d H:i:s");
        echo "<br>";
    }
	...
	echo "</br>";
	echo "</br>";
	echo "ACCOUNT BALAMCE ==".$account_balance;
	echo "</br>";
    ...
    if ($available_balance < 0) {
        $return = array(
            "status" => 'error',
            "message" => 'Bet Not Confirm Check Exposer Limit And Balance. '.$available_balance.'---'.$account_balance.'----'. number_format(abs($total_exposure), 2, ".", ""), //'Insufficient Funds.',
        );
        save_rejection_log($conn, $user_id, ($return['message'] . " #" . __line__), $market_runner_name, $page_call_time, '', '', '', $log_game_type);
        echo json_encode($return);
        exit();
    ...
    if (!empty($fetch_parent_ids['net_exposure_limit']) && $fetch_parent_ids['net_exposure_limit'] < abs($total_exposure)) {
        ...
        $return = array(
            "status" => 'error',
            "message" => 'Bet Not Confirm Check Exposer Limit And Balance.'.$account_balance.'--' . number_format(abs($exposure_balance), 2, ".", ""), //'Insufficient Funds.',
        );
        save_rejection_log($conn, $user_id, ($return['message'] . " #" . __line__), $market_runner_name, $page_call_time, '', '', '', $log_game_type);
        echo json_encode($return);
        exit();
    ...
    $data_API_Fancy11 =  json_decode($data);
    ...
    if ($user_id == 12352 && $print_value) {
        echo "array_search=" . date("Y-m-d H:i:s");
        echo "<br>";
    }
    ...
    if ($user_id == 12352 && $print_value) {
        echo "BOOKMAKERSMALL_ODDS=" . date("Y-m-d H:i:s");
        echo "<br>";
    }
    ...
    if ($user_id == 12352 && $print_value) {
        echo "BOOKMAKER_ODDS=" . date("Y-m-d H:i:s");
        echo "<br>";
    }
            ...
            if (intVal($match_odds_selection->selectionId) == intVal($marketId)) {
                ...
                if ($type == "No" or $type == "Lay") {
                    ...
                    if ($lay_1 <= $runs && $lay_1 > 1) {
                        ...
                        $return = array(
                            "status" => 'error',
                            "message" => "Bet not confirmed, Odds is Changed.123 $lay_1 $runs",
                            "data" => $match_odds_selection,
                        );
                        save_rejection_log($conn, $user_id, ($return['message'] . " #" . __line__), $market_runner_name, $page_call_time, $data_API, $fancy_data, '', $log_game_type);
                        echo json_encode($return);
                        exit();
    ...
    if ($user_id == 12352 && $print_value) {
        echo "insert_bet=" . date("Y-m-d H:i:s");
        echo "<br>";
    }
    $insert_bet = array(
    ...
    if ($bet_id > 0) {
        ...
        if (in_array($market_type, $all_markets) || in_array($market_type, $all_markets_numbers) || $market_type == "BOOKMAKERSMALL_ODDS" || $market_type == "BOOKMAKER_ODDS" || $market_type == "BOOKMAKER_TIED_ODDS") {
            ...
            if ($user_id == 12352 && $print_value) {
                echo "add_net_exposure_v2=" . date("Y-m-d H:i:s");
                echo "<br>";
            }
        ...
        echo $serno . " = ";
        echo "<pre>";
        print_r($add_exposure_amount);
        echo "</pre>";
        echo "<br>";
        /* exit(); */
    } else {

        $columns = implode(',', array_keys($insert_bet));

        $values  = implode("','", array_values($insert_bet));

        echo $sql = "INSERT INTO bet_details ($columns) VALUES ('$values')";
