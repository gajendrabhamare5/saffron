...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("TEEN1", $casino_lis_db)){
		$return = array(
		"status" => 'error',
...
$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='teen1';");
...
if($marketId == 3){
	$dataa=$conn->query("select * from bet_teen_details where event_id='$eventId' and market_id = '4'");
	if(mysqli_num_rows($dataa) > 0){
		$msgg="Bet not confirm bet already exists on 7 Down Player.";
		 $return = array(
        "status" => 'error',
        "message" => $msgg,
		);
...
if($marketId == 4){
	$dataa=$conn->query("select * from bet_teen_details where event_id='$eventId' and market_id = '3'");
	if(mysqli_num_rows($dataa) > 0){
		$msgg="Bet not confirm bet already exists on 7 Up Player.";
		 $return = array(
        "status" => 'error',
        "message" => $msgg,
		);
...
if($marketId == 5){
	$dataa=$conn->query("select * from bet_teen_details where event_id='$eventId' and market_id = '6'");
	if(mysqli_num_rows($dataa) > 0){
		$msgg="Bet not confirm bet already exists on 7 Down Dealer.";
		 $return = array(
        "status" => 'error',
        "message" => $msgg,
		);
...
if($marketId == 6){
	$dataa=$conn->query("select * from bet_teen_details where event_id='$eventId' and market_id = '5'");
	if(mysqli_num_rows($dataa) > 0){
		$msgg="Bet not confirm bet already exists on 7 Up Dealer.";
		 $return = array(
        "status" => 'error',
        "message" => $msgg,
		);
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='teen1'");
...
$url = CASINO_DATA_URL ."?type=teen1";
$ch = curl_init();
