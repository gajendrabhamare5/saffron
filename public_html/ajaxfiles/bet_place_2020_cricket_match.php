...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("2020_CRICKET", $casino_lis_db)){
		$return = array(
		"status" => 'error',
...
$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='cmatch20';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='cmatch20'");
...
$url = CASINO_DATA_URL ."?type=cmatch20";
$ch = curl_init();
...
$t2_datas = $api_data->t2;
	...
	foreach($t2_datas as $t2_data){
		...
		if($api_sid == $marketId){
			...
			$back1 = $t2_data->b1;
			$lay1 = $t2_data->l1;
			if($bet_status !='ACTIVE' and $bet_status != 'OPEN'){
				$return = array(
					"status"=>'error',
					"message"=>'Bet Suspended',
				);
...
$api_event_id = str_replace("35.","",$api_event_id);
...
$bet_market_name = "";
if($marketId == 1){
	$bet_market_name = "RUN2";
}else if($marketId == 2){
	$bet_market_name = "RUN3";
}else if($marketId == 3){
	$bet_market_name = "RUN4";
}else if($marketId == 4){
	$bet_market_name = "RUN5";
}else if($marketId == 5){
	$bet_market_name = "RUN6";
}else if($marketId == 6){
	$bet_market_name = "RUN7";
}else if($marketId == 7){
	$bet_market_name = "RUN6";
}else if($marketId == 8){
	$bet_market_name = "RUN9";
}else if($marketId == 9){
	$bet_market_name = "RUN10";
}
$bet_market_name = "";
$current_bet = array(
