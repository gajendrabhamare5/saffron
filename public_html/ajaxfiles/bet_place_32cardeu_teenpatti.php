...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("32CARDSB", $casino_lis_db)){
		$return = array(
		"status" => 'error',
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='card32eu';");
...
$url = CASINO_DATA_URL ."?type=card32eu";
$ch = curl_init();
...
$back1 = $api_data->t2[$positin]->b1;
$lay1 = $api_data->t2[$positin]->l1;
/* 
...
$api_event_id = str_replace("29.","",$api_event_id);
...
if($marketId >= 1 and $marketId <= 4){
	...
	$bet_market_name = "BLACK_3";
}else if($marketId == 14){
	$bet_market_name = "RED_3";
}else if($marketId == 25){
	$bet_market_name = "";
}else if($marketId == 26){
	$bet_market_name = "";
}else if($marketId == 27){
	$bet_market_name = "BLACK_RED_2_3";
}else if($marketId >= 15 and $marketId <= 24){
	$bet_market_name = "";
}
