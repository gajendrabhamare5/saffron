...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("AAA2", $casino_lis_db)){
		$return = array(
		"status" => 'error',
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='aaa2';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='aaa2'");
...
$api_event_id = str_replace("27.","",$api_event_id);
...
/* else if($marketId == 4){
	...
	$bet_market_name = "ODD_EVEN_1";
}else if($marketId == 6){
	$bet_market_name = "RED_BLACK";
}else if($marketId == 7){
	$bet_market_name = "RED_BLACK_1";
}else if($marketId == 21){
	$bet_market_name = "OVER_UNDER";
}else if($marketId == 22){
	$bet_market_name = "OVER_UNDER_1";
}else{
	$bet_market_name = "";
} */
