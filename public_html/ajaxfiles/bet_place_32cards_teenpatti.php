...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("32CARDS", $casino_lis_db)){
		$return = array(
		"status" => 'error',
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='card32';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='card32'");
...
$url = CASINO_DATA_URL ."?type=card32";
$ch = curl_init();
...
$api_event_id = str_replace("18.","",$api_event_id);
