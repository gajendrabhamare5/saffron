...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("5_CRICET", $casino_lis_db)){
		$return = array(
		"status" => 'error',
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='cricketv3';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='cricketv3'");
...
$get_exposure_details =  $conn->query("select SUM(exposure_amount) as exposure_data from exposure_details where event_id=$eventId and user_id=$user_id and market_id > 2");
...
if(empty($fancy_exposure_data)){
	$fancy_exposure_data = 0;
}

if($marketId > 2){
	$fancy_exposure_data = $fancy_exposure_data - $margin_used;
}

$exposure_balance = $final_net_exposure + $fancy_exposure_data;
$check_plus_expo = $exposure_balance + $unmatched_exposure_balance;

$url = CASINO_DATA_URL ."?type=cricketv3";
$ch = curl_init();
...
foreach($t2_datas as $t2_data){
	...
	if($api_sid == $marketId){
		...
		$back3 = $t2_data->b1;
		$lay3 = $t2_data->l1;
		if($bet_status != "ACTIVE" || $bet_status != "OPEN"){
			$return = array(
				"status"=>'error',
				"message"=>'Bet Suspended',
			);
...
$api_event_id = str_replace("41.","",$api_event_id);
