...
$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='kbc';");
...
$questiontype = $conn->real_escape_string($_REQUEST['questiontype']);
$nat_1 = json_decode($_REQUEST['nat_1'], true);
$nat_2 = json_decode($_REQUEST['nat_2'], true);
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='kbc'");
...
foreach ($t2_datas as $t2_data) {
    ...
    if ($t2_data->odds) {
                ...
                $back1 = 2;
                $lay1 = 2;
                if ($bet_status == 'ACTIVE' or $bet_status == 'OPEN') {
                    $bet_status = 1;
                }
...
$api_event_id = str_replace("12.", "", $api_event_id);
...
foreach($nat_1 as $keyy=>$val){
    $market_id_new=$nat_2[$keyy];
    $conn->query("insert into `kbc_teen_bet` SET `event_id`='$eventId',`bet_id`='$last_bet_id',`user_id`='$user_id',`market_id`='$market_id_new',`market_name`='$val',bet_type='$questiontype',bet_status='1',`date_time`='$bet_time',`bet_final_result`=''");
}
