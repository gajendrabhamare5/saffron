...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("2020_DRAGON_TIGER", $casino_lis_db)){
		$return = array(
		"status" => 'error',
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='dt20';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='dt20'");
...
$api_event_id = str_replace("25.","",$api_event_id);
...
$bet_market_name = "";
/* if($marketId == 1){
	$bet_market_name = "2020_DRAGON_TIGER";
}else if($marketId == 2){
	$bet_market_name = "2020_DRAGON_TIGER_2";
}else if($marketId == 3){
	$bet_market_name = "2020_DRAGON_TIGER_3";
}else if($marketId == 4){
