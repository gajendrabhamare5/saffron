...
<div class="col-md-12 main-container">
    <style type="text/css">
        ...
        .scorecard {
            ...
            background-image: url('http://allpaanellexch.com/storage/img/scorecard-bg.png');
            position: relative;
                ...
                <form>
                    <ul class="d-inline-block">
                        <li class="form-group active-button m-l-20 d-inline-block BetActives">
                            ...
                            <ul class="sub-button">
                                <li>
                                    <a href="javascript:void(0)" id="btn-bet_active_all" data-status="0">All Deactive</a>
                                </li>
                                <li>
                        ...
                        <li class="form-group active-button m-l-20 d-inline-block FancyActive">
                            <a href="javascript:void(0)" class="btn btn-back active" id="btn-fancy_bet_lock">Fancy Lock</a>
                            <ul class="sub-button">
                                <li>
                        ...
                        <li class="form-group active-button m-l-20 d-inline-block">
                            <a href="javascript:void(0)" class="btn btn-back active" id="modal-btn_userbook">User Book</a>
                        </li>
                        <li class="form-group active-button m-l-20 d-inline-block">
                            <a href="javascript:void(0)" class="btn btn-back active" id="modal-btn_userbookmaker">Bookmarkers Book</a>
                        </li>
                    </ul>
                </form>
            </div>
        ...
        <div class="coupon-card row">
            <div class="col-md-8 main-content">
                <div class="game-heading m-b-5">
                    <span class="card-header-title event_name_heading" id="spn_event_title"><?php echo $event_name;?></span>
                </div>
                <div class="card-content">
				 <?php if($eventType == 4){ ?>
                                    ...
                                    <div id="bm-section">
                                        <table class="table coupon-table table table-bordered m-t-10 " border="0">
                                            <thead>
                                                <tr>
                                                    <th>
                                                        <span class="text-info p-r-5">Min:<span id="bookmaker_min">500</span></span>
                                                        <span class="text-info">Max:<span id="bookmaker_max" >200000</span></span>
                                                    </th>
                                                    <th class="box-w1"></th>
                                                    <th class="box-w1"></th>
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-md-6 fancy-market" id="fancy-market" style="">
                            <div class="fancy-marker-title">
                                <h4>Ball by Ball Session Market</h4> </div>
                            <div id="div_fancy">
                                <table class="table coupon-table table table-bordered m-t-10 ">
                                    <thead>
                                        <tr>
                ...
                <div class="modal-body">
                    <div class="usertypewiseactive-container">
                        <form action="" id="form_userwisebet" method="post" autocomplete="off">
                            <div class="msgContainer" style="display: inline-block; width: 50%; margin-bottom: 5px;text-align: center"></div>
    ...
    <div class="modal fade show" id="user-book">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body ">
                    <div class="userBookContainer"><ul class="mtree transit bubba mtree-header">
                        ...
                        <ul class="mtree transit bubba">
                            ...
                            <li class="mtree-node mtree-closed"><a href="javascript:void(0);">
                                    <table class="table"><tbody><tr> <td>jjm09</td><td>0</td><td>60</td></tr></tbody></table>
                                </a></li>
                        </ul>
...
$('#showtv').on('click',function (){
    ...
    socket.on('connect', function() {
        <?php if($handler->layout['marketId']){?>
            socket.emit('getOddData', {eventId: '<?php echo $handler->layout['marketId']; ?>'});
                            ...
                            if(gameList[i].matchid == eventId){
                                ...
                                <?php if(!$handler->layout['marketId']){?>
                                    if(!odds_socket_active){
                                        socket.emit('getOddData', {eventId: gameList[i].marketid});
                                        odds_socket_active = true;
                                    }
                                <?php } ?>
                            }
                        ...
                        if (args.body.oddEven[0].value.session) {
                            ...
                            for (i = 0; i < args.body.oddEven[0].value.session.length; i++) {
                                ...
                                if (marketId == "" || args.body.oddEven[0].value.session[i].GameStatus.toUpperCase() == "OFFLINE" || args.body.oddEven[0].value.session[i].GameStatus.toUpperCase() == "CLOSED") {
                                    ...
                                    if (statusLabel == "") {
                                        $("#fancyBetMarket_" + eventId + "_" + marketId).removeClass("suspended-fancy");
                                    } else {
                                        $("#fancyBetMarket_" + eventId + "_" + marketId).addClass("suspended-fancy");
                                    }
                                    $("#fancyBetMarket_" + eventId + "_" + marketId).attr("data-title", statusLabel);
...
//                $('#spn_event_title').html(title_html);
                ...
                console.log("event_id=",data.results.event_id);
    ...
    function update_user_bet_status(status, users) {
        $.post(MASTER_URL + '/events_analysis/user_bet_status', {'status': status, users: users,event_id:eid}, function (responce) {
    ...
    function update_user_fancy_bet_status(status, users) {
        $.post(MASTER_URL + '/events_analysis/user_fancy_bet_status', {'status': status, users: users,event_id:eid}, function (responce) {
    ...
    $(function () {
       ...
       /*  setInterval(function (){
        ...
        $('#modal-btn_userbook').on('click',function (){
            ...
            $.get(MASTER_URL + '/events_analysis/get_userbook/' + eid +'/MATCH_ODDS', function (data) {
...
<div class="modal_error"></div>                  
  <div class="matchBookContainer">
				...
				<input type="hidden" name="event_id" id="event_id" value="<?php echo $handler->layout['event_id']; ?>" />
				<input type="hidden" name="event_name_limit" id="event_name_limit" value="" />
				<input type="hidden" name="matchdate" id="matchdate" value="" />
				<input type="hidden" name="oddsmarketId" id="oddsmarketId" value="<?php echo $handler->layout['marketId']; ?>" />
            </div>
			
			<div class="modal-footer pull-right">
                        <button class="btn btn-back" type="button" data-dismiss="modal"><i class="fa fa-undo" aria-hidden="true"></i> Back</button>
                        <button class="btn btn-submit" type="submit" onclick="set_max_limit()"> Submit <i class="fa fa-angle-double-right" aria-hidden="true"></i></button>
                    </div>
