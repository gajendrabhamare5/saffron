...
if ($market_type == "Match Odds" or $market_type == "Toss Odds" or $market_type == "Tie Odds" or $market_type == "Other Odds") {
    ...
    if ($market_type == "Match Odds") {
        ...
        $get_market_type = $conn->query("select * from bet_details where event_id='$event_id' and market_id='$market_id' and market_type NOT IN ('MATCH_ODDS','TOSS_ODDS','TIED_MATCH','FANCY_ODDS','BOOKMAKER_ODDS','BOOKMAKERSMALL_ODDS') and market_type='$db_market_type'");
    ...
    if ($result_status == 0) {
        ...
        $get_win_bet_details_1 = $conn->query("select * from bet_details where event_id='$event_id' and market_id=$market_id and market_type='$market_type_search' and bet_type='$bet_type' and bet_status=0");
        while ($fetch_get_win_bet_details_1 = mysqli_fetch_assoc($get_win_bet_details_1)) {
            ...
            if ($bet_odds < 1) {
                ...
                if ($market_type == "MATCH_ODDS" && empty($winner_name)) {
                    echo "1=== UPDATE `bet_details` SET `winner_name`='$marketName' WHERE event_id='$event_id' and `market_type` = 'MATCH_ODDS'<br><br>";
                    /* $conn->query("UPDATE `bet_details` SET `winner_name`='$marketName' WHERE event_id='$event_id' and `market_type` = 'MATCH_ODDS'"); */
                }
        ...
        $get_loss_bet_details_1 = $conn->query("select * from bet_details where event_id='$event_id' and market_id<>$market_id and market_type='$market_type_search' and bet_type='$bet_type' and bet_status=0");
        ...
        $get_win_bet_details_2 = $conn->query("select * from bet_details where event_id='$event_id' and market_id<>$market_id and market_type='$market_type_search' and bet_type='$bet_type_2' and bet_status=0");
        ...
        $get_loss_bet_details_2 = $conn->query("select * from bet_details where event_id='$event_id' and market_id=$market_id and market_type='$market_type_search' and bet_type='$bet_type_2' and bet_status=0");
    ...
    $get_account_id = $conn->query("select * from commission_master_test where event_id='$event_id'");
    ...
    while ($fetch_get_all_user_commision = mysqli_fetch_assoc($get_all_user_commision)) {
        ...
        if (isset($commision_list[$commision_user_id])) {
            ...
            echo "2===delete from commission_master_test where comm_id='$comm_id' <br><br>";
            echo "3===delete from accounts_test where account_id IN($both_account_id) and game_type=0 <br><br>";
            echo "4===delete from accounts_temp_test where account_id IN($both_account_temp_id) and game_type=0 <br><br>";
            $conn->query("delete from commission_master_test where comm_id='$comm_id'");

            $conn->query("delete from accounts_test where account_id IN($both_account_id) and game_type=0");
            $conn->query("delete from accounts_temp_test where account_id IN($both_account_temp_id) and game_type=0");
        }
        ...
        if (true) {
            if ($comm_amount > 0) {
                ...
                echo "5===update bet_details set bet_comm='$comm_amount' where bet_id=$commission_bet_id <br><br>";
                ...
                foreach ($level_pers as $cradit_user_id => $level_per) {
                    ...
                    if ($cradit_amt > 0) {
                        ...
                        if (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $cradit_amt != 0)) {
                            echo "6===insert into accounts_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`event_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('" . $cradit_user_id . "','$commision_user_id','$account_description_commision','$commission_bet_id','$event_id','$cradit_amt','Credit','10','$entry_transaction_time','1','$transaction_id') <br><br>";
                            $insert_user_account = $conn->query("insert into accounts_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`event_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('" . $cradit_user_id . "','$commision_user_id','$account_description_commision','$commission_bet_id','$event_id','$cradit_amt','Credit','10','$entry_transaction_time','1','$transaction_id')");
                            $account_id_db4 = $conn->insert_id;
                            $account_ids['accounts_test'][] = $account_id_db4;
                        }
                        if (INSERT_ACCOUNTS_TEMP && (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $cradit_amt != 0))) {
                            echo "7===insert into accounts_temp_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`event_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('" . $cradit_user_id . "','$commision_user_id','$account_description_commision','$commission_bet_id','$event_id','$cradit_amt','Credit','10','$entry_transaction_time','1','$transaction_id') <br><br>";
                            $conn->query("insert into accounts_temp_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`event_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('" . $cradit_user_id . "','$commision_user_id','$account_description_commision','$commission_bet_id','$event_id','$cradit_amt','Credit','10','$entry_transaction_time','1','$transaction_id')");
                            $account_id_db3 = $conn->insert_id;
                            $account_ids['accounts_temp_test'][] = $account_id_db3;
                        }
                    }
                }
                $account_id1 = 0;
                ...
                if (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $comm_amount != 0)) {
                    echo "8====insert into accounts_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`event_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$commision_user_id',0,'$account_description_commision','$commission_bet_id','$event_id','$comm_amount','Debit','9','$entry_transaction_time','1','$transaction_id') <br><br>";
                    ...
                    $account_ids['accounts_test'][] = $account_id_db;
                }
                if (INSERT_ACCOUNTS_TEMP && (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $comm_amount != 0))) {
                    echo "9===insert into accounts_temp_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`event_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$commision_user_id',0,'$account_description_commision','$commission_bet_id','$event_id','$comm_amount','Debit','9','$entry_transaction_time','1','$transaction_id') <br><br>";
                    $conn->query("insert into accounts_temp_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`event_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$commision_user_id',0,'$account_description_commision','$commission_bet_id','$event_id','$comm_amount','Debit','9','$entry_transaction_time','1','$transaction_id')");

                    $account_id_db2 = $conn->insert_id;
                    $account_ids['accounts_temp_test'][] = $account_id_db2;
                }
                $account_id2 = $conn->insert_id;


                $both_account_id = implode(",", $account_ids['accounts_test']);
                $both_accounttemp_id = implode(",", $account_ids['accounts_temp_test']);
                echo "10=====insert into commission_master_test (account_id,account_temp_id,user_id,bet_id,event_id,comm_amount,comm_datetime) values('$both_account_id','$both_accounttemp_id','$commision_user_id','$commission_bet_id','$event_id','$comm_amount','$entry_transaction_time')";
                $conn->query("insert into commission_master_test (account_id,account_temp_id,user_id,bet_id,event_id,comm_amount,comm_datetime) values('$both_account_id','$both_accounttemp_id','$commision_user_id','$commission_bet_id','$event_id','$comm_amount','$entry_transaction_time')");
...
function set_result($conn, $bet_id, $result_status, $fetch_bet_details)
{
    ...
    if ($result_status == 'Win') {
        if ($market_type == "MATCH_ODDS" && empty($winner_name)) {
            $query_list[] = "UPDATE `bet_details` SET `winner_name`='$market_name' WHERE event_id='$event_id' and `market_type` = 'MATCH_ODDS'";
    ...
    if ($result_status == 'Win') {
         ...
         */
        $delete_bet_details = $conn->query("delete from accounts_test where user_id=$bet_user_id and bet_id=$bet_id and game_type=0");
        $delete_bet_details = $conn->query("delete from accounts_temp_test where user_id=$bet_user_id and bet_id=$bet_id and game_type=0");
    ...
    /*  $update_bet_status = $conn->query("update bet_details set bet_status=0,bet_result='$user_amount',bet_result_time='$transaction_time' where bet_id=$bet_id and bet_status=1");
     $delete_exposure = $conn->query("delete from exposure_details where event_id='$eventId' and user_id=$bet_user_id and market_type='$market_type'"); */
    //adjust account 
    ...
    if ($result_status == 'Win') {
        ...
        if (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $actual_win_amount2 != 0)) {
            $query_list[] = "insert into accounts_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$bet_user_id','0','$account_description','$bet_id','$actual_win_amount2','Credit','4','$transaction_time','1','$transaction_id')";
            $insert_user_account = $conn->query("insert into accounts_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$bet_user_id','0','$account_description','$bet_id','$actual_win_amount2','Credit','4','$transaction_time','1','$transaction_id')");
        }
        if (INSERT_ACCOUNTS_TEMP && (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $actual_win_amount2 != 0))) {
            $query_list[] = "insert into accounts_temp_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$bet_user_id','0','$account_description','$bet_id','$actual_win_amount2','Credit','4','$transaction_time','1','$transaction_id')";
            $conn->query("insert into accounts_temp_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$bet_user_id','0','$account_description','$bet_id','$actual_win_amount2','Credit','4','$transaction_time','1','$transaction_id')");
        }
        ...
        foreach ($level_pers as $debit_user_id => $level_per) {
            ...
            if (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $debit_amt != 0)) {
                $query_list[] = "insert into accounts_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$debit_user_id','$bet_user_id','$account_descriptionSMDL','$bet_id','$debit_amt','Debit','7','$transaction_time','1','$transaction_id')";
                $insert_user_accountSMDL = $conn->query("insert into accounts_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$debit_user_id','$bet_user_id','$account_descriptionSMDL','$bet_id','$debit_amt','Debit','7','$transaction_time','1','$transaction_id')");
            }
            if (INSERT_ACCOUNTS_TEMP && (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $debit_amt != 0))) {
                $query_list[] = "insert into accounts_temp_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$debit_user_id','$bet_user_id','$account_descriptionSMDL','$bet_id','$debit_amt','Debit','7','$transaction_time','1','$transaction_id')";
                $conn->query("insert into accounts_temp_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$debit_user_id','$bet_user_id','$account_descriptionSMDL','$bet_id','$debit_amt','Debit','7','$transaction_time','1','$transaction_id')");
            }
        }
    } else if ($result_status == 'Loss') {
        $transaction_id = 'sports-' . $bet_id . '-' . $bet_user_id;
        ...
        if (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $user_amount != 0)) {
            $query_list[] = "insert into accounts_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$bet_user_id','0','$account_description','$bet_id','$user_amount','Debit','7','$transaction_time','1','$transaction_id')";

            $insert_user_account = $conn->query("insert into accounts_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$bet_user_id','0','$account_description','$bet_id','$user_amount','Debit','7','$transaction_time','1','$transaction_id')");
        }
        if (INSERT_ACCOUNTS_TEMP && (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $user_amount != 0))) {
            $query_list[] = "insert into accounts_temp_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$bet_user_id','0','$account_description','$bet_id','$user_amount','Debit','7','$transaction_time','1','$transaction_id')";
            $conn->query("insert into accounts_temp_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$bet_user_id','0','$account_description','$bet_id','$user_amount','Debit','7','$transaction_time','1','$transaction_id')");
        }
        if (isset($per_list[$bet_user_id])) {
            $level_pers = $per_list[$bet_user_id];
        } else {
        ...
        foreach ($level_pers as $cradit_user_id => $level_per) {
            ...
            if (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $cradit_amt != 0)) {
                $query_list[] = "insert into accounts_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$cradit_user_id','$bet_user_id','$account_descriptionSMDL','$bet_id','$cradit_amt','Credit','4','$transaction_time','1','$transaction_id')";
                $insert_user_accountSMDL = $conn->query("insert into accounts_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$cradit_user_id','$bet_user_id','$account_descriptionSMDL','$bet_id','$cradit_amt','Credit','4','$transaction_time','1','$transaction_id')");
            }
            if (INSERT_ACCOUNTS_TEMP && (INSERT_ACCOUNTS_ZERO || (INSERT_ACCOUNTS_ZERO == false && $cradit_amt != 0))) {
                $query_list[] = "insert into accounts_temp_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$cradit_user_id','$bet_user_id','$account_descriptionSMDL','$bet_id','$cradit_amt','Credit','4','$transaction_time','1','$transaction_id')";
                $conn->query("insert into accounts_temp_test (`user_id`,`opp_user_id`,`account_description`,`bet_id`,`amount`,`type`,`entry_type`,`account_date_time`,`status`,`transaction_id`) VALUES('$cradit_user_id','$bet_user_id','$account_descriptionSMDL','$bet_id','$cradit_amt','Credit','4','$transaction_time','1','$transaction_id')");
            }
        }
    }
    return true;
