...
function get_net_exposure_during_placing_v3($conn, $user_id, $current_bet)
{
	...
	if ($bet_type == "FANCY_ODDS") {
		...
		if ($check_num_rows <= 0) {
			...
			$total_expo = $this_current_bet_details;
			if($user_id == "11"){
				$this_current_bet_details=$this_current_bet_details['net_exposure_array'];
			}
		}
	...
	if($user_id == "11"){
		//$this_current_bet_details=$this_current_bet_details['net_exposure_array'];
		return array('abc' => $total_expo,'total' => $total_final_exposure + $net_exposure_other_event);
	}
...
function add_net_exposure_v3($conn, $user_id, $current_bet)
{
	...
	if ($bet_type == "FANCY_ODDS") {
		...
		if ($check_num_rows <= 0) {
			...
			if($user_id == "11"){
				$exposure_data=$exposure_data['net_exposure_array'];
			}
...
function add_net_exposure_v2($conn, $user_id, $current_bet)
{
	...
	if(count($current_bet) > 0 && isset($current_bet['other_fancy']) && $current_bet['other_fancy'] == "other"){
		$bet_type_check = true;
		$ptinnn['testcurrent_bet'][]=$current_bet;
	}
	...
	if ($bet_type == "KHADO_ODDS") { 
		...
		if ($check_num_rows <= 0) {
			//insert
			$ptinnn['testcurrent_bet'][]="insert into exposure_details (user_id,event_id,market_type,exposure_amount,exposure_datetime,event_type,max_winning_amount) values('$user_id','$bet_event_id','$bet_type','-$bet_margin_used','$exposure_datetime','$bet_event_type','$bet_win_amount')";
			...
			$exposure_data123 = $exposure_data_all;
			if($user_id == "11"){
		...
		$ptinnn['testcurrent_bet'][]=$exposure_data123;
		return $ptinnn;
	} else {
		...
		if ($bet_market_type2 == "RACE17" || $bet_market_type2 == "NOTENUM" || $bet_market_type2 == "PATTI2" || $bet_market_type2 == "TEENSIN" || ($bet_market_type2 == "ODITEENPATTI" && $bet_market_id > 2) || ($bet_market_type2 == "TEENJOKER" && $bet_market_id > 2)) {
		...
		if ($check_num_rows <= 0) {
			...
			if ($bet_type == "INSTANT_WORLI") {
				...
				if ($bet_market_type2 == "NOTENUM" || $bet_market_type2 == "RACE17" || $bet_market_type2 == "PATTI2" || $bet_market_type2 == "TEENSIN" || $bet_market_type2 == "ODITEENPATTI" || $bet_market_type2 == "TEENJOKER") {
					if(($bet_market_type2 == "ODITEENPATTI" &&  $bet_market_id <= 2) || ($bet_market_type2 == "TEENJOKER" && $bet_market_id <= 2)){
...
function get_net_exposure_match_oods($conn, $user_id, $bet_event_id, $market_type, $bet_type_exposure, $stack, $runs, $current_bet = array(), $is_all_value = 0)
{
    ...
    if ($market_type == "MATCH_ODDS" or $market_type == "BOOKMAKER_ODDS" or $market_type == "BOOKMAKERSMALL_ODDS" or $market_type == "BOOKMAKER_TIED_ODDS" or $bet_type_check == true) {
		...
		$print_val['othermarkets']=$othermarkets;
...
$print_val['other_market_win'][]=$other_market_win;
	...
	$all_markets_expo=$net_exposure_array;
    if ($is_all_value == 0) {
	...
	if(isset($current_bet['number_market_pass']) && $current_bet['number_market_pass'] == "yes"){
		...
		$print_val['all_markets_expo']=$all_markets_expo;
		if($user_id == "11"){
			return $print_val;
		}
...
function get_net_exposure_casino_oods_on_page($conn, $user_id, $bet_event_id, $market_type, $bet_market_name)
{
	...
	$market_ids = array();
	while ($market_ids_array = mysqli_fetch_assoc($market_id)) {
		if($market_type == "TEEN41" or $market_type == "TEEN42" or $market_type == "TEEN1" or $market_type == "ODITEENPATTI" or $market_type == "TEEN6" or $market_type == "TRAP" or $market_type == "PATTI2" or $market_type == "TEEN3" or $market_type == "TEENJOKER" or $market_type == "NOTENUM" or $market_type == "RACE17"){
