...
function get_unmatched_expo($conn, $user_id, $current_bet = array())
{
	...
	if ($fancy_margin == "" || $fancy_margin == "NULL") {
		$fancy_margin = 0;
	}

	$fancy_margin = $fancy_margin * (-1);
	if (empty($fancy_margin)) {
...
function get_net_exposure_casino_market_new($conn, $user_id, $bet_event_id, $market_type, $marketId, $bet_type_exposure, $stack, $runs, $bet_market_name)
{
	...
	if ($bet_market_name != "") {
		if ($bet_market_name == "") {
			$market_id = $conn->query("select * from event_casino_market_id where market_type='$market_type' and market_id in (1,2)");
		} else {
			$market_id = $conn->query("select * from event_casino_market_id where market_type='$market_type' and back_name='$bet_market_name' and market_id in (1,2)");
		}
