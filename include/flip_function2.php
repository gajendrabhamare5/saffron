			...
			if ($bet_type == "INSTANT_WORLI") {
				...
				if ($bet_market_type2 == "NOTENUM" || $bet_market_type2 == "RACE17" || $bet_market_type2 == "PATTI2" || $bet_market_type2 == "TEENSIN" || $bet_market_type2 == "ODITEENPATTI" || $bet_market_type2 == "TEEN62" || $bet_market_type2 == "TEENJOKER" || $bet_market_type2 == "TRAP" || $bet_market_type2 == "MOGAMBO") {
					if (($bet_market_type2 == "ODITEENPATTI" && $bet_market_id <= 2) || ($bet_market_type2 == "TEEN62" && $bet_market_id <= 2) || ($bet_market_type2 == "TEENJOKER" && $bet_market_id <= 2) || ($bet_market_type2 == "TRAP" && $bet_market_id <= 2) || ($bet_market_type2 == "MOGAMBO" && $bet_market_id <= 2)) {
						...
						$exposure_data = get_current_bet_fancy_casino_exposure_other($conn, $user_id, $bet_event_id, $bet_market_id, $bet_market_type2);
						$exposure_data = min($exposure_data);
...
function get_net_exposure_casino_oods_on_page_other($conn, $user_id, $bet_event_id, $market_type, $bet_market_name)
{
	...
	while ($market_ids_array = mysqli_fetch_assoc($market_id)) {
		...
		if ($market_type == "GOAL2") {
			
		if ($market_ids_array['market_id'] >= 1 && $market_ids_array['market_id'] <= 12) {
			$market_ids[] = $market_ids_array['market_id'];
		} elseif ($market_ids_array['market_id'] >= 21 && $market_ids_array['market_id'] <= 25) {
			$market_ids2[] = $market_ids_array['market_id'];
		} elseif ($market_ids_array['market_id'] > 25) {
			$market_ids3[] = $market_ids_array['market_id'];
		}
		}
	...
	if ($market_type == "GOAL2") {
		/* $wheree = " and market_id <= 25"; */
		$wheree = "AND market_id BETWEEN 1 AND 12";
		$wheree2 = "AND market_id BETWEEN 21 AND 25";
		$wheree3 = " and market_id > 25";
	}
	$market_idss = array_unique($market_ids);
	...
	if (count($market_ids3) > 0) {
		$market_idss3 = array_unique($market_ids3);
		foreach ($market_idss3 as $market_ids3) {
			$market_id3 = $market_ids3;



			$get_match_odds_active_bet = $conn->query("select SUM(bet_margin_used) as total_bet_amount, event_id from bet_teen_details where user_id=$user_id and event_id=$event_id and market_type='$bet_market_name' and bet_status=1 $wheree3 GROUP BY event_id");
			$fetch_match_odds_active_bet = mysqli_fetch_assoc($get_match_odds_active_bet);
			$total_bet_amount = $fetch_match_odds_active_bet['total_bet_amount'];

			$event_1_id = $event_id;

			$get_1_win_back_data = $conn->query("select * from  bet_teen_details  where user_id=$user_id and bet_status =1 and event_id=$event_1_id and market_id='$market_id3' and market_type='$bet_market_name' and bet_type='Back' $wheree3");

			$get_1_opp_lay_data = $conn->query("select * from  bet_teen_details  where user_id=$user_id and bet_status =1 and event_id=$event_1_id and market_id<>'$market_id3' and market_type='$bet_market_name' and bet_type='Lay' $wheree3");
...
function get_current_bet_fancy_casino_exposure_other($conn, $user_id, $event_id, $market_id, $bet_market_type2, $current_bet = array())
{
