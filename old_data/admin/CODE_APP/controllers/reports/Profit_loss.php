...
class Profit_loss extends CI_Controller {
    ...
    private function get_profit_loss_data($limit = '', $offset = '') {
        ...
        $this->db->select('SUM(amount) as amount,IF(ac.game_type = 0,b1.event_type,b2.event_type) as event_type,IF(ac.game_type = 0,b1.event_name,b2.event_name) as event_name,IF(ac.game_type = 0,b1.market_type,b2.market_type) as market_type');


        $this->db->where('ac.bet_id !=', 0);
        if ($client_id = $this->input->post('client_name', TRUE)) {
            $this->db->WHERE('ac.user_id', $client_id);
        } else {
        ...
        $this->db->group_by('event_type,market_type');
        $result = $this->db->get();
