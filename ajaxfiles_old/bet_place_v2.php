...
$delay_master_query = $conn->query("select * from bet_delay_master");
...
if ($type != 'No' and $type != 'Yes') {
	...
	save_rejection_log($conn, $user_id, ($return['message'] . " #$type" . __LINE__), $market_runner_name, $page_call_time, '', '', '', $log_game_type);
	echo json_encode($return);
	exit();
}
...
foreach ($data as $match_data) {
	if (strtoupper($market_odd_name) == strtoupper($data_event_name) || $market_odd_name == "MATCH_ODDS" || $data_event_name == "Most Points" || $market_odd_name == "BOOKMAKER_ODDS" || $market_odd_name == "BOOKMAKER_TIED_ODDS") {
...
if ($bet_odds != $odds) {
	$return = array(
		"status" => 'error',
		"message" => "Invalid bets Odds, Please try again $odds $bet_odds",
	);
	save_rejection_log($conn, $user_id, ($return['message'] . " #" . __LINE__), $market_runner_name, $page_call_time, $data_API, $fancy_data, '', $log_game_type);
	echo json_encode($return);
	exit();
