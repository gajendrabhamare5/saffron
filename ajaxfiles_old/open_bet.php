...
if($eventType == "All" or $eventType == ""){		
	...
	if(isset($_REQUEST['markettype'])){
		...
		if($markettype == 1){
			...
			if(mysqli_num_rows($get_all_open_bet_data) > 0){
	...
	if(isset($_REQUEST['markettype'])){
		...
		if($markettype != 1){
			...
			$rtyy="SELECT * FROM bet_teen_details where 1=1 $teen_query and user_id=$user_id and (bet_status=1 $cc) $event_type_selection order by bet_time desc";
			$get_all_open_bet_data2 = $conn->query("SELECT * FROM bet_teen_details where 1=1 $teen_query and user_id=$user_id and (bet_status=1 $cc) $event_type_selection order by bet_time desc");
			if(mysqli_num_rows($get_all_open_bet_data2) > 0){
			while($fetch_open_bet_data2 = mysqli_fetch_assoc($get_all_open_bet_data2)){			
				if(!empty($fetch_open_bet_data2['joker_card']) AND $fetch_open_bet_data2['joker_card'] == "14"){
					$fetch_open_bet_data2['joker_card']="";
				}
