...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("GOAL2", $casino_lis_db)){
		$return = array(
		"status" => 'error',
...
$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='goal2';");
...
$bet_market_name = "";
if(($marketId >= 1 && $marketId <= 12) || ($marketId >= 21 && $marketId <= 25)){
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='goal2''");
...
$url = CASINO_DATA_URL ."?type=goal2";
$ch = curl_init();
