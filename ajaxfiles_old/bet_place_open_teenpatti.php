...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("OPENTEENPATTI", $casino_lis_db)){
		$return = array(
		"status" => 'error',
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='teen8';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='teen8'");
...
$url = CASINO_DATA_URL ."?type=teen8";
$ch = curl_init();
