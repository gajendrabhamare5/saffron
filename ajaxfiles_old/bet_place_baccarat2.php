...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("BACCARAT_2", $casino_lis_db)){
		$return = array(
		"status" => 'error',
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='baccarat2';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='baccarat2'");
$row_counts = mysqli_num_rows($get_check_result);
if($row_counts != 0){
	$return = array(
		"status"=>'error',
		"message"=>'Bet Suspended1.',
	);
	save_rejection_log($conn,$user_id,($return['message']." #".__line__),$market_runner_name,$page_call_time);
	echo json_encode($return);
	exit();
...
$url = CASINO_DATA_URL ."?type=baccarat2";
...
$api_event_id = str_replace("39.","",$api_event_id);
