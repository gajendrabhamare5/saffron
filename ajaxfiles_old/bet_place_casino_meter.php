...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("CASINO_METER", $casino_lis_db)){
		$return = array(
		"status" => 'error',
...
if ($fetch_userdata['bet_status'] == 0 || $fetch_dldata['bet_status'] == 0 || $fetch_mdldata['bet_status'] == 0 || $fetch_smdldata['bet_status'] == 0|| $fetch_kingdata['bet_status'] == 0) {
	...
	$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='cmeter';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='cmeter'");
...
if($market_runner_name == "Low"){
	$check_market_runner_name = "High";
}else{
	$check_market_runner_name = "Low";
}
$check_high_low = $conn->query("select * from bet_teen_details where event_id='$eventId'  and event_type='$eventType' and market_name='$check_market_runner_name'");
$fetch_check_high_low = mysqli_fetch_assoc($check_high_low);
if($fetch_check_high_low != null){
	$return = array(
		"status"=>'error',
		"message"=>'You can not place a bet on both side,',
	);
	save_rejection_log($conn,$user_id,($return['message']." #".__line__),$market_runner_name,$page_call_time);
	echo json_encode($return);
	exit();
...
$url = CASINO_DATA_URL ."?type=cmeter";
$ch = curl_init();
...
$api_event_id = str_replace("33.","",$api_event_id);
