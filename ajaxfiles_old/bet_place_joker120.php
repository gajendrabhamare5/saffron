...
if (in_array("JOKER120", $casino_lis_db)) {
	$return = array(
		"status" => 'error',
...
$joker_card = $conn->real_escape_string($_REQUEST['joker_card']);
...
$check_bet_exist = $conn->query("select * from bet_teen_details where event_id=$eventId and market_type='$market_type' and user_id=$user_id");
if(mysqli_num_rows($check_bet_exist) > 0){
	$check_bet_exist_data = mysqli_fetch_assoc($check_bet_exist);
	$db_joker=$check_bet_exist_data['joker_card'];
	if($db_joker!=$joker_card){
		$return = array(
			"status" => 'error',
...
$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='joker120';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='joker120'");
...
$url = CASINO_DATA_URL . "?type=joker120";
$ch = curl_init();
