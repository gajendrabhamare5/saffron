...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("INSTANT_WORLI", $casino_lis_db)){
		$return = array(
		"status" => 'error',
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='worli2'");
...
if (strpos($market_runner_name, 'Even') !== false  or strpos($market_runner_name, 'Odd') !== false or strpos($market_runner_name, 'Line') !== false) {
    $stack = $stack * 5;
	$bet_total_amount = $stack;
}


$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='worli2';");
...
$url = CASINO_DATA_URL ."?type=worli2";
$ch = curl_init();
...
$api_event_id = str_replace("23.","",$api_event_id);
...
if($type == "No" or $type == "Lay"){
	...
	$bet_win_amount = $stack2;
	$wiiningamount = $stack2;
}else{
	$exposure_bet_type = "Back";
	$bet_win_amount = $stack2 * ($runs - 1);
	if (strpos($market_runner_name, 'Even') !== false  or strpos($market_runner_name, 'Odd') !== false or strpos($market_runner_name, 'Line') !== false) {
		$bet_win_amount = $bet_win_amount / 2;
	}
	$margin_used = $stack;
