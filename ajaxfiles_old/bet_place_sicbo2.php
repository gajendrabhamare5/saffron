...
$casino_lis_db=array_column($casino_maintanance_list_data, 0);
	...
	if(in_array("SICBO2", $casino_lis_db)){
		$return = array(
		"status" => 'error',
...
$get_blockeventdata = $conn->query("SELECT * FROM `block_event` WHERE UserId='$user_id' and casino_name='sicbo2';");
...
$get_check_result = $conn->query("select * from twenty_teenpatti_result where event_id='$eventId' and game_type='sicbo2'");
...
$url = CASINO_DATA_URL ."?type=sicbo2";
$ch = curl_init();
...
if($type == "No"){
    ...
    if($marketId == "1" || $marketId == "2" || $marketId == "4" || $marketId == "5" || ((int)$marketId >= 47 and (int)$marketId <= 52)){
