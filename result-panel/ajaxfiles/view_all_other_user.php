...
$_POST = check_variable_for_datatable($conn, $_POST);
	...
	$smdl_id = $_POST['user_id'];
	if($user_type == 3){
		$search_type = "parent_id";
		$heading_label = "MDl";
	}else if($user_type == 2){
		$search_type = "parent_id";
		$heading_label = "DL";
...
if($searchValue != ''){
   $searchQuery = " and (ulm.Email_ID like '%".$searchValue."%' or 
	...
	$sel = $conn->query("select count(*) as allcount from user_master as um left outer join user_login_master as ulm on ulm.UserId=um.Id where 1=1 and um.$search_type=$smdl_id");
...
$totalRecords = $records['allcount'];
	...
	$sel = $conn->query("select count(*) as allcount from user_master as um left outer join user_login_master as ulm on ulm.UserId=um.Id where 1=1 and um.$search_type=$smdl_id $searchQuery");
...
$totalRecordwithFilter = $records['allcount'];
	...
	$empRecords = $conn->query("select * from user_master as um left outer join user_login_master as ulm on ulm.UserId=um.Id where 1=1 and um.$search_type=$smdl_id ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage);
...
while ($user_data = mysqli_fetch_assoc($empRecords)) {
	...
	$login_as_html = "";
	if($UserType == 1){
		$login_as_html = " <a href='#' class='btn btn-dark btn-xs' style='float: right;' id='login_as_new_user' user_type='$UserType' user_id='$UserID' >Login as $Email_ID</a>";
	}
		$data[] = array( 
		  ...
		  "Email_ID1"=>$view_all_other_html,
		  "Email_ID2"=>$login_as_html,
