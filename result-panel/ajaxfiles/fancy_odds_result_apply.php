...
foreach($white_list_data as $white_list_url){
	...
	$url = $white_list_url."controller-agent/ajaxfiles/fancy_odds_result_apply_auto.php?event_id=$event_id&market_id=$market_id&market_type=$market_type&match_odds_result=$match_odds_result";
 ...
 */
	...
	if($market_type == "Over"){
		...
		echo json_encode($return_array);
	}else if($market_type == "lottery"){
		
		
		$mmtype = $_REQUEST['mmtype'];
		/* $url = "http://rrexchange247.com/api/auto_result.php?marketId=$market_id&eventId=$event_id&runs=$match_odds_result";
		$url = str_replace(" ","%20",$url);
		file_get_contents($url); */
		...
		if($match_odds_result == "CAN"){
			$delete_bet_details  = $conn->query("update bet_details set bet_status=2 where event_id='$event_id' and market_type='$mmtype'");
			$delete_exposure = $conn->query("delete from exposure_details where event_id='$event_id' and market_type='$mmtype'");
			$return_array = array(
							"status"=>'ok',
							"message"=>'Bet Cancelled',
							);
		...
		$get_win_bet_details = $conn->query("select * from bet_details where event_id='$event_id' and market_type='$mmtype' and bet_status=1");
		while($fetch_get_win_bet_details = mysqli_fetch_assoc($get_win_bet_details)){
			$bet_id = $fetch_get_win_bet_details['bet_id'];
			$bet_type = $fetch_get_win_bet_details['bet_type'];
			$bet_runs = $fetch_get_win_bet_details['bet_runs'];
			...
			$lastDigit = $match_odds_result % 10;
			if(($bet_type == "Yes" or $bet_type == "Back") and $winnner == $lastDigit){
				set_result($conn,$bet_id,'Win',$match_odds_result);
			}else if(($bet_type=="No" or $bet_type == "Lay") and $winnner != $lastDigit){
				set_result($conn,$bet_id,'Win',$match_odds_result);
			}else{
				set_result($conn,$bet_id,'Loss',$match_odds_result);
			}
