<?php
	...
	$current_date =  date('Y-m-d', strtotime('-15 day', strtotime(date("Y-m-d"))));
	$wheree=" and date(bet_time)>='$current_date'";
														
	$get_active_market_name = $conn->query("select  * from bet_details where event_type='$event_type' and bet_status=2 $wheree GROUP BY event_id,oddsmarketId");
