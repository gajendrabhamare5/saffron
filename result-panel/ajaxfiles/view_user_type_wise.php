...
if($searchValue != ''){
   $searchQuery = " and (ulm.Email_ID like '%".$searchValue."%' or 
	...
	$sel = $conn->query("select count(*) as allcount from user_master as um left outer join user_login_master as ulm on ulm.UserId=um.Id where ulm.UserType=$user_type ");
...
$totalRecords = $records['allcount'];
	...
	$sel = $conn->query("select count(*) as allcount from user_master as um left outer join user_login_master as ulm on ulm.UserId=um.Id where ulm.UserType=$user_type  $searchQuery");
...
$totalRecordwithFilter = $records['allcount'];
	...
	$empRecords = $conn->query("select * from user_master as um left outer join user_login_master as ulm on ulm.UserId=um.Id where ulm.UserType=$user_type  ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage);
...
while ($user_data = mysqli_fetch_assoc($empRecords)) {
	...
	$get_exposure_balance = $conn->query("select SUM(`exposure_amount`) as total_exposure from exposure_details where user_id=$UserID");
	$fetch_get_exposure_balance = mysqli_fetch_assoc($get_exposure_balance);
	$total_exposure = $fetch_get_exposure_balance['total_exposure'];
	if($total_exposure == null or $total_exposure == ""){
		$total_exposure =0;
	}
