...
$check_user_name = $conn->query("select * from user_login_master where `Email_ID`='$username' and UserType IN (10)");
...
if ($user_password_db == $salted_hash) {
	...
	if ($Stat == "1") {
		...
		if (true) {
			...
			$_SESSION['CONTROLLER_LOGIN_SESSION_TYPE'] = 5;
			$_SESSION['CONTROLLER_CONTROLLER_TYPE'] = $controller_type;
			$_SESSION['login_key_chkk'] = $Password2_salt;
